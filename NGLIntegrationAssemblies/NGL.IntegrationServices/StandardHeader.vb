...
Public Class StandardHeaders
    ...
    Public Function ReadFromFile(ByVal fileName As String, Optional ByVal Details As List(Of Object) = Nothing) As List(Of Object) Implements IHeaders.ReadFromFile
                    ...
                    Try
                        ...
                        If strRow.Length >= 29 Then
                            ...
                            If Not Details Is Nothing AndAlso Details.Count > 0 Then
                                Dim HDetails As List(Of Object) = (From d In Details Where d.ItemPONumber = H.PONumber).ToList()
                                H.Details = New StandardDetails
                                H.Details.Items = HDetails
                            End If
                            Items.Add(H)
                        Else
                            Errors &= "StandardHeader.ReadFromFile Error; invalid row not enough columns:" & vbCrLf & "     " & r & vbCrLf & vbCrLf
                        End If
                    Catch ex As ApplicationException
                        Errors &= "StandardHeader.ReadFromFile parse CSV Error: " & ex.Message & vbCrLf & vbCrLf
                    End Try
                Next
            Else
                Errors &= errMsg
        ...
        Catch ex As Exception
            Errors &= "StandardHeader.ReadFromFile Unexpected Error: " & ex.Message & vbCrLf & vbCrLf
        End Try
        Return Items
    ...
    Public Function buildBookObjectData(ByVal vHeaders As IHeaders, _
        ...
        Try
            ...
            For Each H As StandardHeader In vHeaders.Items

                Dim oH As New NGLBookWebService.clsBookHeaderObject
                With oH
                    .PONumber = H.PONumber
                    .POVendor = H.POVendor
                    .POdate = CastToDate(H.POdate, Date.Now).ToShortDateString
                    .POShipdate = CastToDate(H.POShipdate, Date.Now).ToShortDateString
                    .POBuyer = H.POBuyer
                    ...
                    .POReqDate = CastToDate(H.POReqDate, Date.Now).ToShortDateString
                ...
                For Each D As StandardDetail In H.Details.Items
...
Public Class StandardHeader
    ...
    Public Details As StandardDetails
