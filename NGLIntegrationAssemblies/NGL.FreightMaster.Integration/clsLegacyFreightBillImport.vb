...
<Serializable()> _
Public Class clsLegacyFreightBillImport : Inherits clsDownload
...
#End Region
    ...
    Public Function getDataSet() As LegacyFreightBillData
        Return New LegacyFreightBillData
    End Function

    Public Function ProcessObjectData( _
                        ByVal oLegacyFreightBills() As clsLegacyFreightBillObject, _
        ...
        Dim oHTable As New LegacyFreightBillData.LegacyFreightBillDataTable
        Dim dtVal As Date
        Try
            For Each oItem As clsLegacyFreightBillObject In oLegacyFreightBills
                Dim oRow As LegacyFreightBillData.LegacyFreightBillRow = oHTable.NewLegacyFreightBillRow
                With oRow
                ...
                End With
                oHTable.AddLegacyFreightBillRow(oRow)
            Next

            intRet = ProcessData(oHTable, strConnection)
        Catch ex As Exception
            LogException("Process Object Data Failure", "Order import system error", AdminEmail, ex, "NGL.FreightMaster.Integration.clsLegacyFreightBillImport.ProcessObjectData Failure")
        End Try
        Return intRet
    ...
    Public Function ProcessData( _
                ByVal oLegacyFreightBills As LegacyFreightBillData.LegacyFreightBillDataTable, _
                ByVal strConnection As String) As ProcessDataReturnValues
        ...
        Me.ItemName = ""
        Me.ImportTypeKey = IntegrationTypes.LegacyFreightBillImport
        Me.CreatedDate = Now.ToString
        Me.CreateUser = "Data Integration DLL"
        Me.Source = "Legacy Freight Bill Data Integration"
        ...
        Try
            ...
            importHeaderRecords(oLegacyFreightBills, oFields)
            ...
            Log(strMsg)
        Catch ex As Exception
            LogException("Process Payables Data Failure", "Could not process the requested payables data.", AdminEmail, ex, "NGL.FreightMaster.Integration.clsLegacyFreightBillImport.ProcessData")
        Finally
            closeConnection()
        End Try
        Return intRet
    ...
    Private Function importHeaderRecords( _
        ByRef oLegacyFreightBills As LegacyFreightBillData.LegacyFreightBillDataTable, _
        ByRef oFields As clsImportFields) As Boolean
        Dim Ret As Boolean = False
        Try

            'now get the LegacyFreightBill Header Records
            Dim intRetryCt As Integer = 0
            Dim strSource As String = "clsLegacyFreightBillImport.importHeaderRecords"
            ...
            Do
                ...
                Try
                    ...
                    Try
                        Dim lngMax As Long = oLegacyFreightBills.Count
                        Log("Importing " & lngMax & " LegacyFreightBill Header Records.")
                        For Each oRow As LegacyFreightBillData.LegacyFreightBillRow In oLegacyFreightBills
                            strErrorMessage = ""
                ...
                Catch ex As Exception
                    If intRetryCt > Me.Retry Then
                        ITEmailMsg &= "<br />" & Source & " Failure: NGL.FreightMaster.Integration.clsLegacyFreightBillImport.importHeaderRecords, attempted to import payables header records from Data Integration DLL " & intRetryCt.ToString & " times without success.<br />" & vbCrLf & readExceptionMessage(ex) & "<br />" & vbCrLf
                        Log("NGL.FreightMaster.Integration.clsLegacyFreightBillImport.importHeaderRecords Failed!" & readExceptionMessage(ex))
                    Else
                        Log("Import freight bill records failure Retry = " & intRetryCt.ToString)
                    End If
                End Try
        ...
        Catch ex As Exception
            ITEmailMsg &= "<br />" & Source & " Failure: NGL.FreightMaster.Integration.clsLegacyFreightBillImport.importHeaderRecords, Could not import from Data Integration DLL.<br />" & vbCrLf & readExceptionMessage(ex) & "<br />" & vbCrLf
            Log("NGL.FreightMaster.Integration.clsLegacyFreightBillImport.importHeaderRecords Failed!" & readExceptionMessage(ex))
        End Try
        Return Ret
    ...
    Private Function buildHeaderCollection(ByRef oFields As clsImportFields) As Boolean
        ...
        Try
            ...
            For ct As Integer = 1 To oFields.Count
                ...
                Try
                    If oFields(ct).Name = "APCarrierNumber" Or oFields(ct).Name = "APBillNumber" Then
                        blnUseField = True
                    Else
                        blnUseField = getImportFieldFlag(oFields(ct).Name, IntegrationTypes.LegacyFreightBillImport)
                    End If
            ...
            Ret = True
        Catch ex As Exception
            ITEmailMsg &= "<br />" & Source & " Failure: NGL.FreightMaster.Integration.clsLegacyFreightBillObject.buildHeaderCollection, could not build the header collection data.<br />" & vbCrLf & readExceptionMessage(ex) & "<br />" & vbCrLf
            Log("NGL.FreightMaster.Integration.clsLegacyFreightBillObject.buildHeaderCollection Failed!" & readExceptionMessage(ex))
        End Try
        Return Ret
    ...
    Public Overloads Function SaveData(ByRef oFields As clsImportFields) As Boolean
        ...
        Try
                ...
                Try
                    ...
                    If Me.openConnection Then
                        ...
                        With cmd
                            ...
                            If Not oFields("APBillDate").Value.ToUpper = "NULL" Then .Parameters("@APBillDate").Value = oFields("APBillDate").Value
                            ...
                            .Parameters.Add("@APElectronicFlag", SqlDbType.Money)
                            ...
                            .CommandText = "dbo.spInsertFreightBill"
                        ...
                        Try
                            If intErrNumber <> 0 Then
                                If intRetryCt > Me.Retry Then
                                    Me.RecordErrors += 1
                                    ITEmailMsg &= "<br />" & Source & " Failure: clsLegacyFreightBillImport.saveData: Procedure spInsertFreightBill output failure: " _
                                        & vbCrLf & "Error # " & intErrNumber & ": " & strRetMsg & ".  Could not import freight bill record.<br />" & vbCrLf
                                    Log("saveData Failed!")
                                    Exit Do
                                Else
                                    Log("NGL.FreightMaster.Integration.clsLegacyFreightBillImport.saveData Output Failure Retry = " & intRetryCt.ToString)
                                End If
                            Else
                            ...
                            End If
                        Catch ex As Exception
                            Log("NGL.FreightMaster.Integration.clsLegacyFreightBillImport.saveData Unexpected Error " & readExceptionMessage(ex) & vbCrLf & " Retry = " & intRetryCt.ToString)
                        End Try
                    Else
                        If intRetryCt > Me.Retry Then
                            Me.RecordErrors += 1
                            ITEmailMsg &= "<br />" & Source & " Failure: clsLegacyFreightBillImport.saveData: Open database connection failure, attempted to create a database connection " & intRetryCt.ToString & " times without success.  Could not import freight bill record.<br />" & vbCrLf
                            Log("saveData Failed!")
                            Exit Do
                        Else
                            Log("NGL.FreightMaster.Integration.clsLegacyFreightBillImport.saveData Open DB Connection Failure Retry = " & intRetryCt.ToString)
                        End If

                    End If
                Catch ex As Exception
                    If intRetryCt > Me.Retry Then
                        ITEmailMsg &= "<br />" & Source & " Failure: clsLegacyFreightBillImport.saveData unexpected error. Could not import freight bill record:<br />" & vbCrLf & readExceptionMessage(ex) & vbCrLf & " Retry = " & intRetryCt.ToString & "<br />" & vbCrLf
            ...
            Loop Until intRetryCt > Me.Retry 'this should never happen the code is here to show our intention.
        Catch ex As Exception
            ITEmailMsg &= "<br />" & Source & " Failure: clsLegacyFreightBillImport.saveData: Could not import freight bill record.<br />" & vbCrLf & readExceptionMessage(ex) & "<br />" & vbCrLf
            Log("saveData Failed!" & readExceptionMessage(ex))
        End Try
        Return blnRet
