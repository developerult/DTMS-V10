...
Public Class clsImportFields
    ...
    Private mList As New List(Of clsImportField)

    Public Function GetEnumerator() As System.Collections.IEnumerator Implements System.Collections.IEnumerable.GetEnumerator
        Return mCol.GetEnumerator
    End Function
    Public Function Add(ByVal oField As clsImportField) As clsImportField

        If oField.Key.Trim.Length > 0 Then
            mCol.Add(oField, oField.Key)
        Else
            mCol.Add(oField)
        End If
        If mList Is Nothing Then mList = New List(Of clsImportField)
        mList.Add(oField)
    ...
    Public Sub Remove(ByRef vntIndexKey As Object)
        ...
        'Syntax: x.Remove(xyz)
        Dim oItem As clsImportField = mCol.Item(vntIndexKey)
        If containsKey(oItem.Key) Then
            mList.Remove(oItem)
        End If
    ...
    Public Function containsKey(ByVal sKey As String) As Boolean
        If mList Is Nothing OrElse mList.Count < 1 Then Return False
        Return mList.Any(Function(x) x.Key = sKey)
    End Function

    Public Function containsName(ByVal sName As String) As Boolean
        If mList Is Nothing OrElse mList.Count < 1 Then Return False
        Return mList.Any(Function(x) x.Name = sName)
    End Function

    Public Sub New()
        MyBase.New()
        mCol = New Collection
        mList = New List(Of clsImportField)
    End Sub
