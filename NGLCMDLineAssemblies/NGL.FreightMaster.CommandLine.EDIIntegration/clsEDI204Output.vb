...
Public Class clsEDI204Output : Inherits EDIFileCore
    ...
    Public Overrides Function Read(Optional ByVal strErrMsg As String = "") As Boolean
        ...
        With o204
            ...
            enumResults = .getTruckLoadEDI204String(FileData, ConnectionString, CarrierNumber, FileName, sConfigFile:=OutboundConfigFileName) 'Modified by LVV 3/2/16 for v-7.0.5.1 EDI Migration
            strLastError = .LastError
        End With


        If enumResults <> NGLWebReturnValues.nglDataIntegrationComplete Then
            Dim strMsg As String = strErrMsg & " read 204 truckload data failed for carrier " & CarrierNumber & " using file " & FileName
            ...
            End Select
            LogError(Source & " Read EDI 204 Data Failure", strMsg, Me.AdminEmail)
        Else
            blnRet = True
