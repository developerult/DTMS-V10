...
Public Class clsBookold : Inherits clsDownload
    ...
    Public Sub FileImport(ByRef strHeaderFile As String, ByRef strItemFile As String, ByRef strDataPath As String)
        ...
        Do
            ...
            Try
                ...
                If Not FMLib.dbUtilities.OpenConnection() Then
                    If intRetryCt > Me.Retry Then
                        LogError("PODownload Failure", "PODownload.clsBook.FileImport: Order download open database connection failure" & vbCrLf & "FMLib.dbUtilities.OpenConnection Returned False 3 times.", GroupEmail)
                        Exit Sub
                    Else
                        Log("FileImport OpenConnection Failed Retry = " & intRetryCt.ToString)
                    End If
                End If
                Exit Do 'The database conection is open.
            Catch ex As Exception
                If intRetryCt > Me.Retry Then
                    LogError("PODownload Failure", "PODownload.clsBook.FileImport: Order download open database connection failure" & vbCrLf & ex.ToString, GroupEmail)
                    Exit Sub
                Else
                    Log("FileImport Download Open Database Connection Failure Retry = " & intRetryCt.ToString)
                End If
            End Try
            'We only get here if an exception is thrown and intRetryCt <= 3
        Loop Until intRetryCt > Me.Retry 'this should never happen the code is here to show our intention.
        ...
        Try
            ...
            Do Until objImportRS.EOF
                ...
                If FillHeaderData(PO, objImportRS, blnVError, strErrorMessage, strSource) Then
                    ...
                    Else
                        If Val(PO.POConsigneeNumber) > 0 Then
                            UpdateLaneNumber(PO)
                        End If
                        blnInsertRecord = False
                        'test if the record already exists. 
                        If searchForExistingHeader(PO, blnInsertRecord) Then
                            'import the data stored in the PO class
                            If processOrder(blnInsertRecord, PO) Then
                                lngImportCount = lngImportCount + 1
                                'Update POHDR Defaults
                                If updatePOHDRDefaults(PO) Then
                                    'We only update hash totals if all processes are successfull
                                    mintTotalOrders += 1
                                    mdblHashTotalOrders += Val(stripQuotes(PO.POOrderNumber))
                                End If
                            End If
                        End If
                    End If
        ...
        Catch ex As Exception
            LogError("PODownload Failure", "PODownload.clsBook.FileImport: Order download system error" & vbCrLf & ex.ToString, GroupEmail)
            Exit Sub
        Finally
            Try
                objImportRS = Nothing
        ...
        Try
            ...
            Do Until objImportRS.EOF
                ...
                If FillItemData(PI, objImportRS, blnVError, strErrorMessage, strSource) Then
                    ...
                    Else
                        blnInsertRecord = False
                        'test if the ItemNumber is null if so we must perform an insert because no key is provided.
                        If PI.ItemPONumber = "Null" Then
                            blnInsertRecord = True
                        End If
                        'test if the record already exists if not inserting a new record 
                        If blnInsertRecord Or searchForExistingItem(PI, blnInsertRecord) Then
                            'import the data stored in the PI class
                            If processItems(blnInsertRecord, PI) Then
        ...
        Try
            ...
            If lngErrCt > 0 Then
                strMsg = strMsg & vbCrLf & vbCrLf & "ERROR!  " & lngErrCt _
                    & " PO Header records could not be imported.  " _
                    & "Please run the PO Header Import Error Report for more information."
            End If
            ...
            If lngImportItemErrors > 0 Then
                strMsg = strMsg & vbCrLf & vbCrLf & "ERROR!  " & lngImportItemErrors _
                    & " PO Item records could not be imported.  " _
                    & "Please run the PO Item Import Error Report for more information."
            End If
            Log(strMsg)
        Catch ex As Exception
            Log("Error Creating Log Message." & vbCrLf & ex.ToString)
        End Try
    End Sub


    Public Sub LogHeaderImportError(ByVal PO As clsPOHeader, ByVal strErrorMessage As String)
        ...
        Try
            ...
            strSQL = strSQL & " Values " _
                ...
                & "," & padSpaces(strErrorMessage) & "," & padSpaces(PO.POPallets) & "," & padSpaces(PO.OtherCosts) & "," & padSpaces(PO.StatusFlag) & ")"
    ...
    Public Sub LogItemImportError(ByVal PI As clsPOItem, ByVal strErrorMessage As String)
    ...
    Public Function validateFieldData(ByRef blnVError As Boolean, _
        ...
        Try
            strRetVal = validateSQLValue(objrs _
                , enm _
                , strSource _
                , strErrMsg _
                , blnAllowNull _
    ...
    Public Function FillHeaderData(ByRef PO As clsPOHeader _
        ...
        Try
            ' Field # 1
            PO.POOrderNumber = validateFieldData(blnVError _
                , strErrorMessage _
...
GetCompNumberByAlpha:
            ...
            PO.POCustomerPO = validatePossibleFieldData("", objImportRS.Fields("POCustomerPO") _
                ...
                , 20)
            ' Field # 28
            PO.OtherCosts = validatePossibleFieldData("0", objImportRS.Fields("POOtherCosts") _
                , ValidateDataType.vdtFloat _
                , strSource _
                , "Invalid POOtherCosts" _
                , True)
            ' Field # 29
            PO.StatusFlag = validatePossibleFieldData("0", objImportRS.Fields("POStatusFlag") _
                , ValidateDataType.vdtLongInt _
                , strSource _
                , "Invalid POStatusFlag" _
                , True)
            Try
                If PO.POOrderNumber.Length < 1 And PO.POCustomerPO.Length > 0 Then
                    PO.POOrderNumber = PO.POCustomerPO
                End If
        ...
        Catch ex As Exception
            LogError("PODownload Failure", "PODownload.clsBook.FillHeaderData: Order download fill POHeader data failure; order number " & PO.POOrderNumber & " failed to import" & vbCrLf & ex.ToString, AdminEmail)
            Return False
        End Try
    End Function

    Public Function FillItemData(ByRef PI As clsPOItem _
        , ByRef objImportRS As ADODB.Recordset _
        , ByRef blnVError As Boolean _
        , ByRef strErrorMessage As String _
        , ByRef strSource As String) As Boolean
        Try
            ...
            PI.CustItemNumber = validateFieldData(blnVError _
                ...
                , 50)
            ' Field # 20
            PI.ItemCustomer = validateFieldData(blnVError _
                , strErrorMessage _
...
GetCompNumberByAlpha:
            Try
                Dim objTestRS As New ADODB.Recordset
                Dim strSQL As String = "Select dbo.udfGetCompNumberByAlpha(" & PI.ItemCustomer & ") as CompNumber"
                With objTestRS
                    .Open(strSQL, gDBCon, ADODB.CursorTypeEnum.adOpenStatic, ADODB.LockTypeEnum.adLockReadOnly)
                    If .RecordCount > 0 Then
                        ' Validate New Value
                        PI.ItemCustomer = validateFieldData(blnVError _
                            ...
                            , True)
                    Else
                        PI.ItemCustomer = Val(PI.ItemCustomer)
                    End If
                    .Close()
                End With
            Catch ex As Exception
                intRetryCt += 1
                If intRetryCt > Me.Retry Then
                    LogError("PODownload Failure", "PODownload.clsBook.FillItemData: Order detail download failure call udfGetCompNumberByAlpha Error; order item number " & PI.ItemNumber & " failed to import" & vbCrLf & ex.ToString, AdminEmail)
                    Return False
                Else
                    Log("FillItemData udfGetCompNumberByAlpha Failure Retry = " & intRetryCt.ToString)
                    GoTo GetCompNumberByAlpha
                End If
            End Try
