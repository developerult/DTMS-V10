...
Public Class clsSchedule : Inherits clsDownload
    ...
    Private Function importHeaderRecords( _
            ...
            Do
                        ...
                        Do Until objImportRS.EOF
                            ...
                            Select Case intRet
                                Case -1
                                    ...
                                    strErrorMessage = "Could not check for existing schedule information in the book table.  The order number " & oFields("BookCarrOrderNumber").Value.ToString & " has not been downloaded."
                                Case 0 'no records found so we cannot update the schedule data
                                    blnValidationError = True
                                    strErrorMessage = "Could import schedule data the order number " & oFields("BookCarrOrderNumber").Value.ToString & ", it does not have a valid booking record and has not been downloaded."
                                Case Else
                ...
                Catch ex As Exception
                    If intRetryCt > Me.Retry Then
                        LogError("PODownload Failure", "PODownload.clsSchedule.importHeaderRecords: could not import from file " & strHeaderFile & vbCrLf & ex.ToString & vbCrLf & strSQL, AdminEmail)
                    Else
        ...
        Catch ex As Exception
            LogError("PODownload Failure", "PODownload.clsSchedule.importHeaderRecords: could import from file " & strHeaderFile & vbCrLf & ex.ToString, AdminEmail)
        End Try
        Return Ret
    ...
    Private Function buildHeaderCollection(ByRef oFields As clsImportFields) As Boolean
        ...
        Try
            With oFields
                ...
                .Add("BookCarrScheduleDate", "BookCarrScheduleDate", clsImportField.DataTypeID.gcvdtDate, 20, True)
                .Add("BookCarrScheduleTime", "BookCarrScheduleTime", clsImportField.DataTypeID.gcvdtTime, 20, True)
                .Add("BookCarrActualDate", "BookCarrActualDate", clsImportField.DataTypeID.gcvdtDate, 20, True)
                .Add("BookCarrActualTime", "BookCarrActualTime", clsImportField.DataTypeID.gcvdtTime, 20, True)
                .Add("BookCarrActLoadCompleteDate", "BookCarrActLoadCompleteDate", clsImportField.DataTypeID.gcvdtDate, 20, True)
                .Add("BookCarrActLoadCompleteTime", "BookCarrActLoadCompleteTime", clsImportField.DataTypeID.gcvdtTime, 20, True)
            End With
            ...
            For ct As Integer = 1 To oFields.Count
                ...
                Try
                    blnUseField = CBool(getImportFieldFlag(oFields(ct).Name, ImportFileTypes.ietSchedule))
                Catch ex As Exception
            ...
            Ret = True
        Catch ex As Exception
            LogError("PODownload Failure", "PODownload.clsSchedule.buildHeaderCollection failure" & vbCrLf & ex.ToString, AdminEmail)
        End Try
        Return Ret
