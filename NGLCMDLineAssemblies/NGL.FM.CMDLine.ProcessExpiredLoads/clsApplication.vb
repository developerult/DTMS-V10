...
Public Class clsApplication : Inherits NGL.FreightMaster.Core.NGLCommandLineBaseClass
    ...
    Public Function getWebDefaults(ByVal CompControl As Integer) As Tuple(Of Integer, String, String)
        ...
        Catch ex As Exception
            ...
            'fixed bug where email to address was wrong.
            LogError("Load Expired Failure", "Load Expired Calling Stored Proc " & vbCrLf & sErrMsg & vbCrLf & vbCrLf & Me.NEXTRackDatabase & vbCrLf & Me.NEXTRackDatabaseServer, Me.AdminEmail)
    ...
    ''' <summary>
    ''' Moved some code from here in CMDLine project to the new bll method ProcessExpiredLoads365()
    ''' Also got rid of call to getWebDefaults()
    ''' </summary>
    ''' <remarks>
    ''' Added By LVV on 1/31/2019
    ''' </remarks>
    Public Sub ProcessExpirations365()
        Try
            Dim result As DTO.WCFResults = bookbll.ProcessExpiredLoads365(True, True)
            If result IsNot Nothing AndAlso result.Success = False Then
                ''TODO email messages from result get actrion result.
                ''failed. lets send an email.
                'result.Actio
