<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="CarrierInsuranceExpired.aspx.cs" Inherits="DynamicsTMS365.CarrierInsuranceExpired" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head >
    <title>DTMS Carrier Insurance Expired Alerts</title>        
         <%=cssReference%>             
        <style>
            html,
    ...
    <% Response.WriteFile("~/Views/HelpWindow.html"); %>   
     ...
     <script>
        ...
        var PageControl = '<%=PageControl%>'; 
         var oCarrierInsuranceExpiredGrid = null;
         var wnd = kendo.ui.Window; 
         var tObj = this;
         var tPage = this;
         var iCarrierInsuranceExpiredPK = 0;
        ...
        <% Response.Write(NGLOAuth2); %>
                             ...
                             if (data.Data.length > 0 && typeof (data.Data[0]) !== 'undefined' && ngl.isObject(data.Data[0])) {
                                 ...
                                 if(typeof (userPageSettings) !== 'undefined' && userPageSettings != null && userPageSettings.value != undefined){
                                     if (userPageSettings.name === "pk"){ 
                                         if(typeof (iCarrierInsuranceExpiredPK) === 'undefined' || iCarrierInsuranceExpiredPK == null || iCarrierInsuranceExpiredPK === 0){ iCarrierInsuranceExpiredPK = userPageSettings.value; }                                 
                                     }                                                                                                                                  
                                 }                                   
                             }                              
                         }                            
         ...
         function saveCarrierInsuranceExpiredPK() {
             try {
                 var row = oCarrierInsuranceExpiredGrid.select();
                 ...
                 var dataItem = oCarrierInsuranceExpiredGrid.dataItem(row);
                 ...
                 if ("RunTaskControl" in dataItem){
                     ...
                     iCarrierInsuranceExpiredPK = dataItem.RunTaskControl;
                     var setting = {name:'pk', value: iCarrierInsuranceExpiredPK.toString()};
                     var oCRUDCtrl = new nglRESTCRUDCtrl();
                     var blnRet = oCRUDCtrl.update("CarrierInsuranceExpired/PostPageSetting", setting, tPage, "savePostPageSettingSuccessCallback", "savePostPageSettingSuccessCallbackAjaxErrorCallback",tPage);
                     return true;
                 } else {
                     ngl.showValidationMsg("TMS Task Required", "Please select a TMS Task Record to continue", tPage);
                     return false;
         ...
         function isCarrierInsuranceExpiredSelected() {
             if (typeof (iCarrierInsuranceExpiredPK) === 'undefined' || iCarrierInsuranceExpiredPK === null || iCarrierInsuranceExpiredPK === 0) {
                 return saveCarrierInsuranceExpiredPK();
             }
             return true;
         }
         function execActionClick(btn, proc){            
             if(btn.id == "btnRefresh" ){ 
                 oCarrierInsuranceExpiredGrid.dataSource.read();
             } else if(btn.id == "btnResetCurrentUserConfig"){ resetCurrentUserConfig(PageControl); }
         }
         var blnCarrierInsuranceExpiredGridChangeBound = false;
         //*************  Call Back Functions ****************
         function CarrierInsuranceExpiredGridDataBoundCallBack(e,tGrid){           
             oCarrierInsuranceExpiredGrid = tGrid;
             if (blnCarrierInsuranceExpiredGridChangeBound == false){
                 oCarrierInsuranceExpiredGrid.bind("change", saveCarrierInsuranceExpiredPK);
                 blnCarrierInsuranceExpiredGridChangeBound = true;
             }
             //if iCarrierInsuranceExpiredPK is not 0 select that row in the grid
             if (typeof (iCarrierInsuranceExpiredPK) !== 'undefined' && iCarrierInsuranceExpiredPK !== null && iCarrierInsuranceExpiredPK !== 0) {
                 var rows = oCarrierInsuranceExpiredGrid.items();
                 $(rows).each(function(e) {
                     var row = this;
                     var dataItem = oCarrierInsuranceExpiredGrid.dataItem(row);
                     if (dataItem.RunTaskControl == iCarrierInsuranceExpiredPK) { 
                         oCarrierInsuranceExpiredGrid.select(row); 
                         return false;
                     }
                 });
             }
             ...
             //lblBalanceID =  wdgtCarrierInsuranceExpiredGridEdit.GetFieldID("lblBalanceTotal");
             //wdgtCarrierInsuranceExpiredGridEdit.SetFieldDefault("lblBalanceTotal",'$100.00');
         ...
         $(document).ready(function () {
            ...
            if (control != 0){
                ...
                getPageSettings(tPage, "CarrierInsuranceExpired", "pk", false);
            }
