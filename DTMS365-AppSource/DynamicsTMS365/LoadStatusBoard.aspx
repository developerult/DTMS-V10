<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="LoadStatusBoard.aspx.cs" Inherits="DynamicsTMS365.LoadStatusBoard" %>

<!DOCTYPE html>

<html>
<head>
    <title>DTMS Load Status Board</title>
    <%=cssReference%>
    <style>
        html,
        body {
    ...
    <div id="example" style="height: 100%; width: 100%; margin-top: 2px;">
        ...
        <% Response.Write(PageCustomJS); %>
            ...
            function getBookProNumber() {
                try {
                    LoadStatusBoardGridSelectedRow = oLoadStatusBoardGrid.select();
                    if (typeof (LoadStatusBoardGridSelectedRow) === 'undefined' || LoadStatusBoardGridSelectedRow == null) { ngl.showValidationMsg("Booking Record Required", "Please select a Booking to continue", tPage); return false; }
                    LoadStatusBoardGridSelectedRowDataItem = oLoadStatusBoardGrid.dataItem(LoadStatusBoardGridSelectedRow); //Get the dataItem for the selected row
                    if (typeof (LoadStatusBoardGridSelectedRowDataItem) === 'undefined' || LoadStatusBoardGridSelectedRowDataItem == null) { ngl.showValidationMsg("Booking Record Required", "Please select a Booking to continue", tPage); return false; }
                    if ("BookProNumber" in LoadStatusBoardGridSelectedRowDataItem) {
                        //debugger;
                        var blnUpdateSummary = true;
                        var iOldBookPK = iBookPK;
                        iBookPK = LoadStatusBoardGridSelectedRowDataItem.BookControl;
                        if ("BookConsPrefix" in LoadStatusBoardGridSelectedRowDataItem) {
                            sSelectedCNS = LoadStatusBoardGridSelectedRowDataItem.BookConsPrefix;
                        }
                        if (iBookPK == iOldBookPK) { blnUpdateSummary = false; };
                        var setting = { name: 'pk', value: iBookPK.toString() };
                        var oCRUDCtrl = new nglRESTCRUDCtrl();
                        var blnRet = oCRUDCtrl.update("LoadStatusBoard/PostPageSetting", setting, tPage, "savePostPageSettingSuccessCallback", "savePostPageSettingAjaxErrorCallback", tPage);
                        ...
                        if ("BookTranCode" in LoadStatusBoardGridSelectedRowDataItem) {
                            var tranCode = LoadStatusBoardGridSelectedRowDataItem.BookTranCode;
            ...
            function saveBookPK() {
                try {
                    ...
                    LoadStatusBoardGridSelectedRowDataItem = oLoadStatusBoardGrid.dataItem(LoadStatusBoardGridSelectedRow); //Get the dataItem for the selected row
                    if (typeof (LoadStatusBoardGridSelectedRowDataItem) === 'undefined' || LoadStatusBoardGridSelectedRowDataItem == null) { ngl.showValidationMsg("Booking Record Required", "Please select a Booking to continue", tPage); return false; }
                    if ("BookControl" in LoadStatusBoardGridSelectedRowDataItem) {
                        //debugger;
                        var blnUpdateSummary = true;
                        var iOldBookPK = iBookPK;
                        iBookPK = LoadStatusBoardGridSelectedRowDataItem.BookControl;
            ...
            function execActionClick(btn, proc) {
                //debugger;          
                if (btn.id == "btnAddBooking") { openAddNewLoadStatusBoardGridWindow(); } 
                    ...
                    if (isBookingSelected() === true) {
                        if ("BookODControl" in LoadStatusBoardGridSelectedRowDataItem) {
                            
                            var lanePK = LoadStatusBoardGridSelectedRowDataItem.BookODControl;
                    ...
                    //debugger;
                    if (isBookingSelected() === true) {
                        if ("BookCustCompControl" in LoadStatusBoardGridSelectedRowDataItem) {
                            var compPK = LoadStatusBoardGridSelectedRowDataItem.BookCustCompControl;
                    ...
                    if (typeof (tPage[proc]) !== 'undefined' && ngl.isObject(tPage[proc])) {
                        ...
                        //tPage[proc].close();
                        //oLoadStatusBoardGrid.dataSource.read();
                        //if (blnShowRateItAfterBookingOptions === true) {
                        //    var iThisBookControl = iBookPK;
                        //    execActionClick("Refresh");
                        //    var oRow = oLoadStatusBoardGrid.table.find('tr[data-id="' + iBookPK + '"]');
                        //    if (typeof (oRow) !== 'undefined' && ngl.isObject(oRow)) {
                        //        oLoadStatusBoardGrid.select(oRow);
                        //        saveBookPK();
                    ...
                    if (isBookingSelected() === true) {
                        if ("BookODControl" in LoadStatusBoardGridSelectedRowDataItem) {
                            var LaneControl = LoadStatusBoardGridSelectedRowDataItem.BookODControl;
                    ...
                    if (isBookingSelected() === true) {
                        if ("BookCustCompControl" in LoadStatusBoardGridSelectedRowDataItem){
                            var compControl = LoadStatusBoardGridSelectedRowDataItem.BookCustCompControl;                           
                    ...
                    var intGridRecords = oLoadStatusBoardGrid.dataSource.total();
            ...
            function openLoadTenderReport() {
                var pro = "";
                if ("BookProNumber" in LoadStatusBoardGridSelectedRowDataItem) { pro = LoadStatusBoardGridSelectedRowDataItem.BookProNumber; }
            ...
            function createNewClaim() {
                ...
                kendo.ui.progress(oLoadStatusBoardGrid.element, true);
                var oCRUDCtrl = new nglRESTCRUDCtrl();
            ...
            function refresh() { ngl.readDataSource(oLoadStatusBoardGrid); }

            var blnLoadStatusBoardGridChangeBound = false;
            //*************  Call Back Functions ****************
            // Modified by CHA v-8.5 on 06/22/2021
            // Example of how callbacks can be used

            function LoadStatusBoardGridDragStartCallBack(draggedObject, event) {
                //debugger;
                //console.log('Drag start - LoadStatusBoard page!');
            ...
            function LoadStatusBoardGridDropCallBack(draggedObject, droppedObject, isDraggingUp) {
                //console.log('Drop - LoadStatusBoard page!');
                //console.log(draggedObject);
                ...
                const Orders = new BookLoadStatusBoardDropped();
                Orders.draggedObject = draggedObject;
                Orders.droppedObject = droppedObject
                Orders.isDraggingUp = isDraggingUp;
                var oCRUDCtrl = new nglRESTCRUDCtrl();
                var blnRet = oCRUDCtrl.update("LoadStatusBoard/PostGridDrop", Orders, tPage, "PostGridDropSuccessCallback", "PostGridDropAjaxErrorCallback", false);

            }
            function BookLoadStatusBoardDropped() {
                draggedObject: null;
                droppedObject: null;
                isDraggingUp: false;              

            }
            var sLoadStatusBoardGridKey = '';
            function LoadStatusBoardGridDataBoundCallBack(e, tGrid, sKey) {
                sLoadStatusBoardGridKey = sKey;
                oLoadStatusBoardGrid = tGrid;
                if (blnLoadStatusBoardGridChangeBound == false) {
                    oLoadStatusBoardGrid.bind("change", saveBookPK);
                    blnLoadStatusBoardGridChangeBound = true;
                }
                ...
                if (typeof (iBookPK) !== 'undefined' && iBookPK !== null && iBookPK !== 0) {
                    var rows = oLoadStatusBoardGrid.items();
                    $(rows).each(function (e) {
                        var row = this;
                        var dataItem = oLoadStatusBoardGrid.dataItem(row);
                        if (dataItem.BookControl == iBookPK) {
                            oLoadStatusBoardGrid.select(row);
                            //Modified by RHR for v-8.5.4.001 on 05/30/2023 updated the selected LoadStatusBoardGridSelectedRowDataItem with the current record.
                            // updated some other dependent variables as well.
                            LoadStatusBoardGridSelectedRowDataItem = dataItem;
                            if ("BookConsPrefix" in LoadStatusBoardGridSelectedRowDataItem) {
                                sSelectedCNS = LoadStatusBoardGridSelectedRowDataItem.BookConsPrefix;
                            }                           
                            //check if the TranCode is PB or PC and enable or disable the Print BOL action button (We can print the BOL for both PB or PC)  
                        ...
                        //    var newRows = oLoadStatusBoardGrid.table.find('tr');
                        ...
                        //    //var cRow = oLoadStatusBoardGrid.table.find('tr[data-id="' + dataItem.BookControl + '"]');
            ...
            function AcceptAll(iRet) {
                ...
                if (!ngl.isNullOrWhitespace(sLoadStatusBoardGridKey)) {
                    var sfilterFuncionname = "obj" + sLoadStatusBoardGridKey + "Filters"
                    var s = window[sfilterFuncionname].data;
                    kendo.ui.progress($("#center-pane"), true);
                    setTimeout(function (tObj) {
                        var oCRUDCtrl = new nglRESTCRUDCtrl();
                        var blnRet = oCRUDCtrl.filteredRead("LoadStatusBoard/AcceptAllRecords", s, tPage, "AcceptAllSuccessCallback", "AcceptAllAjaxErrorCallback", false);
                    }, 10, tObj);
                } else {
            ...
            function getAvailableBookingActions() {
                ...
                if ("BookControl" in LoadStatusBoardGridSelectedRowDataItem) { bkControl = LoadStatusBoardGridSelectedRowDataItem.BookControl; }
                if ("BookTranCode" in LoadStatusBoardGridSelectedRowDataItem) { tranCode = LoadStatusBoardGridSelectedRowDataItem.BookTranCode; }
                if ("BookDateOrdered" in LoadStatusBoardGridSelectedRowDataItem) { dtOrdered = LoadStatusBoardGridSelectedRowDataItem.BookDateOrdered; }
                if ("BookDateRequired" in LoadStatusBoardGridSelectedRowDataItem) { dtRequired = LoadStatusBoardGridSelectedRowDataItem.BookDateRequired; }
                if ("BookDateLoad" in LoadStatusBoardGridSelectedRowDataItem) { dtLoad = LoadStatusBoardGridSelectedRowDataItem.BookDateLoad; }
                if ("BookFinARInvoiceDate" in LoadStatusBoardGridSelectedRowDataItem) { dtInvoice = LoadStatusBoardGridSelectedRowDataItem.BookFinARInvoiceDate; }
                if ("BookCarrierControl" in LoadStatusBoardGridSelectedRowDataItem) { carrierControl = LoadStatusBoardGridSelectedRowDataItem.BookCarrierControl; }
                if ("BookLockAllCosts" in LoadStatusBoardGridSelectedRowDataItem) { lockAllCosts = LoadStatusBoardGridSelectedRowDataItem.BookLockAllCosts; }
                if ("BookSHID" in LoadStatusBoardGridSelectedRowDataItem) { shid = LoadStatusBoardGridSelectedRowDataItem.BookSHID; }
                if ("BookDateDelivered" in LoadStatusBoardGridSelectedRowDataItem) { dtDelivered = LoadStatusBoardGridSelectedRowDataItem.BookDateDelivered; }
                if ("BookRouteConsFlag" in LoadStatusBoardGridSelectedRowDataItem) { routeConsFlag = LoadStatusBoardGridSelectedRowDataItem.BookRouteConsFlag; }
                if ("NEXTStop" in LoadStatusBoardGridSelectedRowDataItem) { blnNEXTStop = LoadStatusBoardGridSelectedRowDataItem.NEXTStop; }
                if ("DAT" in LoadStatusBoardGridSelectedRowDataItem) { blnDAT = LoadStatusBoardGridSelectedRowDataItem.DAT; }
            ...
            function readyToAssignCarrier() {
                //Get the dataItem for the selected row
                var iRet = 1;
                if (typeof (LoadStatusBoardGridSelectedRowDataItem) === 'undefined' || LoadStatusBoardGridSelectedRowDataItem == null) { ngl.showValidationMsg("Booking Record Required", "Please select a Booking to continue", tPage); return -1; }
                var bkControl = 0, carrierControl = 0, routeConsFlag = 0;
                var tranCode = "", dtOrdered = "", dtRequired = "", dtLoad = "", dtInvoice = "", dtDelivered = "", shid = "";
                ...
                if ("BookDateDelivered" in LoadStatusBoardGridSelectedRowDataItem) { dtDelivered = LoadStatusBoardGridSelectedRowDataItem.BookDateDelivered;  }
                if ("BookRouteConsFlag" in LoadStatusBoardGridSelectedRowDataItem) { routeConsFlag = LoadStatusBoardGridSelectedRowDataItem.BookRouteConsFlag;}
                if ("OnCreditHold" in LoadStatusBoardGridSelectedRowDataItem) { blnOnCreditHold = LoadStatusBoardGridSelectedRowDataItem.OnCreditHold; } //Added By LVV on 7/14/20 for v-8.3.0.001 Task #20200609155542 - Credit Hold
            ...
            function BookingTenderWndCB(oResults) {
                if (oResults.widget.sNGLCtrlName === "wdgtBookingTenderWndDialog" && oResults.source === "saveSuccessCallback") {
                    ...
                    //        var oGrid1 = $('#id621201809251302564013007')  //.kendoNGLGrid tPage.oLoadStatusBoardGrid;
                    //        var oGrid = tPage.oLoadStatusBoardGrid;
                    //        //var data = oGrid.data("kendoNGLGrid");
                    //        var oRow6 = tPage.oLoadStatusBoardGrid.table.find('tr[data-id="' + iThisBookControl + '"]')
                    //        if ("BookControl" in oRow6) {
                    //            //tPage.LoadStatusBoardGridSelectedRowDataItem = oRow1;
                    ...
                    //                oGrid.select(oRow);
                    //                //tPage.saveBookPK();
                    //                tPage.LoadStatusBoardGridSelectedRowDataItem = oRow;// oLoadStatusBoardGrid.dataItem(LoadStatusBoardGridSelectedRow);
                    //                if (typeof (tPage.LoadStatusBoardGridSelectedRowDataItem) === 'undefined' || tPage.LoadStatusBoardGridSelectedRowDataItem == null) {
                    //                    ngl.showValidationMsg("Cannot Auto Select the Previous Booking Record ", "Please select a Booking to continue", tPage);
                    //                    return false;
                    //                }
                    //                if ("BookControl" in tPage.LoadStatusBoardGridSelectedRowDataItem) {
                    //                    tPage.iBookPK = tPage.LoadStatusBoardGridSelectedRowDataItem.BookControl;
                    //                    var setting = { name: 'pk', value: iBookPK.toString() };
                    //                    var oCRUDCtrl = new nglRESTCRUDCtrl();
                    //                    var blnRet = oCRUDCtrl.update("LoadStatusBoard/PostPageSetting", setting, tPage, "savePostPageSettingSuccessCallback", "savePostPageSettingAjaxErrorCallback", tPage);
                    //                } else {
            ...
            function SelectTransLoad(e) {
                ...
                if (isBookingSelected() === true) {
                    ...
                    setTimeout(function (tObj) {
                        var oCRUDCtrl = new nglRESTCRUDCtrl();
                        var blnRet = oCRUDCtrl.filteredRead("LoadStatusBoard/GetTransLoad", s, tPage, "GetTransLoadSuccessCallback", "GetTransLoadAjaxErrorCallback", false);
                    }, 10, tObj);
                }
                ...
                //        kendo.ui.progress(oLoadStatusBoardGrid.element, true);
                //        var oCRUDCtrl = new nglRESTCRUDCtrl();
                //        var blnRet = oCRUDCtrl.filteredPost("LoadStatusBoard/HoldLoad", iRowBookControl.toString(), tPage, "PostHoldBookingSuccessCallback", "PostHoldBookingAjaxErrorCallback", false);
                //    } else {
                ...
                //        kendo.ui.progress(oLoadStatusBoardGrid.element, true);
                //        var oCRUDCtrl = new nglRESTCRUDCtrl();
                //        var blnRet = oCRUDCtrl.filteredPost("LoadStatusBoard/UnHoldLoad", iRowBookControl.toString(), tPage, "PostHoldBookingSuccessCallback", "PostHoldBookingAjaxErrorCallback", false);
            ...
            function BingMapsCaller() {
                var wgt = 0;
                if ("BookTotalWgt" in LoadStatusBoardGridSelectedRowDataItem) { wgt = LoadStatusBoardGridSelectedRowDataItem.BookTotalWgt; }
                MapIt(iBookPK, wgt);
            }

            function coreErrorCallBack(xhr, textStatus, error, errTitle) {
                //debugger;
                kendo.ui.progress(oLoadStatusBoardGrid.element, false);
            ...
            function stopResequence(iBookPK) {
                kendo.ui.progress(oLoadStatusBoardGrid.element, true);
                var oCRUDCtrl = new nglRESTCRUDCtrl();
                var blnRet = oCRUDCtrl.read("LoadStatusBoard/StopResequence", iBookPK, tPage, "stopResequenceSuccessCallback", "stopResequenceAjaxErrorCallback", true);
            }
            //Start ResetAPExport
            ...
            function ResetAPExport(iBookPK) {
                kendo.ui.progress(oLoadStatusBoardGrid.element, true);
                var oCRUDCtrl = new nglRESTCRUDCtrl();
                var blnRet = oCRUDCtrl.filteredPost("LoadStatusBoard/ResetAPExport", iBookPK.toString(), tPage, "ResetAPExportSuccessCallback", "ResetAPExportAjaxErrorCallback", true);
            }
              //end
            ...
            function ConfirmShip(iRet, iBookPK) {
                ...
                kendo.ui.progress(oLoadStatusBoardGrid.element, true);
                var oCRUDCtrl = new nglRESTCRUDCtrl();
                var blnRet = oCRUDCtrl.filteredPost("LoadStatusBoard/ConfirmShip", iBookPK.toString(), tPage, "ConfirmShipSuccessCallback", "ConfirmShipAjaxErrorCallback", true);
            }
            ...
            function getMiles(iBookPK) {
                kendo.ui.progress(oLoadStatusBoardGrid.element, true);
                var oCRUDCtrl = new nglRESTCRUDCtrl();
                var blnRet = oCRUDCtrl.read("LoadStatusBoard/GetMiles", iBookPK, tPage, "getMilesSuccessCallback", "getMilesAjaxErrorCallback", true);
            }
            ...
            function DuplicatePro(iBookPK) {
                if (isBookingSelected() === true) {
                    kendo.ui.progress(oLoadStatusBoardGrid.element, true);
                    var oCRUDCtrl = new nglRESTCRUDCtrl();
                    var blnRet = oCRUDCtrl.read("LoadStatusBoard/DuplicatePro", iBookPK, tPage, "DuplicateProSuccessCallback", "DuplicateProAjaxErrorCallback", true);
            ...
            function NewSequence(iBookPK) {
                if (isBookingSelected() === true) {
                    kendo.ui.progress(oLoadStatusBoardGrid.element, true);
                    var oCRUDCtrl = new nglRESTCRUDCtrl();
                    var blnRet = oCRUDCtrl.read("LoadStatusBoard/createNewSequence", iBookPK, tPage, "NewSequenceSuccessCallback", "NewSequenceAjaxErrorCallback", true);
                }
            }
            ...
            function createTransLoad(iBookPK) {
                if (isBookingSelected() === true) {
                    kendo.ui.progress(oLoadStatusBoardGrid.element, true);
                    var oCRUDCtrl = new nglRESTCRUDCtrl();
                    var blnRet = oCRUDCtrl.read("LoadStatusBoard/createTransLoad", iBookPK, tPage, "createTransLoadSuccessCallback", "createTransLoadAjaxErrorCallback", true);
                }
            }
            ...
            function Calculate(iBookPK) {
                if (isBookingSelected() === true) {
                    kendo.ui.progress(oLoadStatusBoardGrid.element, true);
                    var oCRUDCtrl = new nglRESTCRUDCtrl();
                    var blnRet = oCRUDCtrl.read("LoadStatusBoard/Calculate", iBookPK, tPage, "CalculateSuccessCallback", "CalculateAjaxErrorCallback", true);
                }
            }
            ...
            function RecalculateUsingLineHaul(iBookPK) {
                if (isBookingSelected() === true) {
                    kendo.ui.progress(oLoadStatusBoardGrid.element, true);
                    var oCRUDCtrl = new nglRESTCRUDCtrl();
                    var blnRet = oCRUDCtrl.read("LoadStatusBoard/RecalculateUsingLineHaul", iBookPK, tPage, "RecalculateUsingLineHaulSuccessCallback", "RecalculateUsingLineHaulAjaxErrorCallback", true);
                }
            }
            ...
            function UpdateCNS(iBookPK) {
                if (isBookingSelected() === true) {
                    kendo.ui.progress(oLoadStatusBoardGrid.element, true);
                    var oCRUDCtrl = new nglRESTCRUDCtrl();
                    var blnRet = oCRUDCtrl.read("LoadStatusBoard/UpdateCNS", iBookPK, tPage, "UpdateCNSSuccessCallback", "UpdateCNSAjaxErrorCallback", true);
                }
            }
            ...
            function StampCNS(iBookPK) {
                if (isBookingSelected() === true) {
                    ...
                    if (sStampingCNS.length > 0) {
                        ...
                        if ((LoadStatusBoardGridSelectedRowDataItem.BookTranCode == "N" || LoadStatusBoardGridSelectedRowDataItem.BookTranCode == "P") && LoadStatusBoardGridSelectedRowDataItem.BookHoldLoad == 0) {
                    ...
                    var blnRet = oCRUDCtrl.filteredRead("LoadStatusBoard/getCNSStamp", filter, tPage, "StampCNSSuccessCallback", "StampCNSAjaxErrorCallback", false);
                }
            }
            ...
            function PostUpdateCNS(e) {
                ...
                if (iRowBookControl != 0 && (data.BookTranCode == "N" || data.BookTranCode == "P") && data.BookHoldLoad == 0) {
                    ...
                    kendo.ui.progress(oLoadStatusBoardGrid.element, true);
                    var oCRUDCtrl = new nglRESTCRUDCtrl();
                    var blnRet = oCRUDCtrl.filteredPost("LoadStatusBoard/saveStampedCNS", iRowBookControl.toString(), tPage, "PostUpdateCNSSuccessCallback", "PostUpdateCNSAjaxErrorCallback", false);
            ...
            $(document).ready(function () {
                ...
                if (control != 0) {
                    ...
                    getPageSettings(tPage, "LoadStatusBoard", "pk", false);
                } 
