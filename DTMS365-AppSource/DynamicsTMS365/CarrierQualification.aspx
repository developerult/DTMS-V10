<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="CarrierQualification.aspx.cs" Inherits="DynamicsTMS365.CarrierContractExpired" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head >
    <title>DTMS Carrier Qualification</title>        
         <%=cssReference%>             
        <style>
            html,
    ...
    <% Response.WriteFile("~/Views/HelpWindow.html"); %>   
     ...
     <script>
        ...
        var PageControl = '<%=PageControl%>'; 
         var oCarrierContractExpiredGrid = null;
         var wnd = kendo.ui.Window; 
         var tObj = this;
         var tPage = this;
         var iCarrierContractExpiredPK = 0;
        ...
        <% Response.Write(NGLOAuth2); %>
                             ...
                             if (data.Data.length > 0 && typeof (data.Data[0]) !== 'undefined' && ngl.isObject(data.Data[0])) {
                                 ...
                                 if(typeof (userPageSettings) !== 'undefined' && userPageSettings != null && userPageSettings.value != undefined){
                                     if (userPageSettings.name === "pk"){ 
                                         if(typeof (iCarrierContractExpiredPK) === 'undefined' || iCarrierContractExpiredPK == null || iCarrierContractExpiredPK === 0){ iCarrierContractExpiredPK = userPageSettings.value; }                                 
                                     }                                                                                                                                  
                                 }                                   
                             }                              
                         }                            
         ...
         function saveCarrierContractExpiredPK() {
             try {
                 var row = oCarrierContractExpiredGrid.select();
                 ...
                 var dataItem = oCarrierContractExpiredGrid.dataItem(row);
                 ...
                 if ("RunTaskControl" in dataItem){
                     ...
                     iCarrierContractExpiredPK = dataItem.RunTaskControl;
                     var setting = {name:'pk', value: iCarrierContractExpiredPK.toString()};
                     var oCRUDCtrl = new nglRESTCRUDCtrl();
                     var blnRet = oCRUDCtrl.update("CarrierContractExpired/PostPageSetting", setting, tPage, "savePostPageSettingSuccessCallback", "savePostPageSettingSuccessCallbackAjaxErrorCallback",tPage);
                     return true;
                 } else {
                     ngl.showValidationMsg("TMS Task Required", "Please select a TMS Task Record to continue", tPage);
                     return false;
         ...
         function isCarrierContractExpiredelected() {
             if (typeof (iCarrierContractExpiredPK) === 'undefined' || iCarrierContractExpiredPK === null || iCarrierContractExpiredPK === 0) {
                 return saveCarrierContractExpiredPK();
             }
             return true;
         }
         function execActionClick(btn, proc){            
             if(btn.id == "btnRefresh" ){ 
                 oCarrierContractExpiredGrid.dataSource.read();
             } else if(btn.id == "btnResetCurrentUserConfig"){ resetCurrentUserConfig(PageControl); }
         }
         var blnCarrierContractExpiredGridChangeBound = false;
         //*************  Call Back Functions ****************
         function CarrierContractExpiredGridDataBoundCallBack(e,tGrid){           
             oCarrierContractExpiredGrid = tGrid;
             if (blnCarrierContractExpiredGridChangeBound == false){
                 oCarrierContractExpiredGrid.bind("change", saveCarrierContractExpiredPK);
                 blnCarrierContractExpiredGridChangeBound = true;
             }
             //if iCarrierContractExpiredPK is not 0 select that row in the grid
             if (typeof (iCarrierContractExpiredPK) !== 'undefined' && iCarrierContractExpiredPK !== null && iCarrierContractExpiredPK !== 0) {
                 var rows = oCarrierContractExpiredGrid.items();
                 $(rows).each(function(e) {
                     var row = this;
                     var dataItem = oCarrierContractExpiredGrid.dataItem(row);
                     if (dataItem.RunTaskControl == iCarrierContractExpiredPK) { 
                         oCarrierContractExpiredGrid.select(row); 
                         return false;
                     }
                 });
             }
             ...
             //lblBalanceID =  wdgtCarrierContractExpiredGridEdit.GetFieldID("lblBalanceTotal");
             //wdgtCarrierContractExpiredGridEdit.SetFieldDefault("lblBalanceTotal",'$100.00');
         ...
         $(document).ready(function () {
            ...
            if (control != 0){
                ...
                getPageSettings(tPage, "CarrierContractExpired", "pk", false);
            }
