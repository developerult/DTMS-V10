<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="CarrierExposureShipment.aspx.cs" Inherits="DynamicsTMS365.CarrierExposureShipment" %>
...
<head >
    <title>DTMS Carrier Exposure per Shipment</title>        
         <%=cssReference%>             
        <style>
            html,
    ...
    <% Response.WriteFile("~/Views/HelpWindow.html"); %>   
     ...
     <script>
        ...
        var PageControl = '<%=PageControl%>'; 
         var oCarrierExposureShipmentGrid = null;
         var wnd = kendo.ui.Window; 
         var tObj = this;
         var tPage = this;
         var iCarrierExposureShipmentPK = 0;
        ...
        <% Response.Write(NGLOAuth2); %>
         ...
         function savePostPageSettingSuccessCallback(results){
             wdgtCarrierExposureShipment.read(iCarrierExposureShipmentPK);
         }
                             ...
                             if (data.Data.length > 0 && typeof (data.Data[0]) !== 'undefined' && ngl.isObject(data.Data[0])) {
                                 ...
                                 if(typeof (userPageSettings) !== 'undefined' && userPageSettings != null && userPageSettings.value != undefined){
                                     if (userPageSettings.name === "pk"){ 
                                         if(typeof (iCarrierExposureShipmentPK) === 'undefined' || iCarrierExposureShipmentPK == null || iCarrierExposureShipmentPK === 0){ iCarrierExposureShipmentPK = userPageSettings.value; }                                 
                                     }                                                                                                                                  
                                 }                                   
                             }                              
                         }                            
         ...
         function saveCarrierExposureShipmentPK() {
             try {
                 var row = oCarrierExposureShipmentGrid.select();
                 ...
                 var dataItem = oCarrierExposureShipmentGrid.dataItem(row);
                 ...
                 if ("RunTaskControl" in dataItem){
                     ...
                     iCarrierExposureShipmentPK = dataItem.RunTaskControl;
                     var setting = {name:'pk', value: iCarrierExposureShipmentPK.toString()};
                     var oCRUDCtrl = new nglRESTCRUDCtrl();
                     var blnRet = oCRUDCtrl.update("CarrierExposureShipment/PostPageSetting", setting, tPage, "savePostPageSettingSuccessCallback", "savePostPageSettingSuccessCallbackAjaxErrorCallback",tPage);
                     return true;
                 } else {
                     ngl.showValidationMsg("TMS Task Required", "Please select a TMS Task Record to continue", tPage);
                     return false;
         ...
         function isCarrierExposureShipmentSelected() {
             if (typeof (iCarrierExposureShipmentPK) === 'undefined' || iCarrierExposureShipmentPK === null || iCarrierExposureShipmentPK === 0) {
                 return saveCarrierExposureShipmentPK();
             }
             return true;
         }
         function execActionClick(btn, proc){            
             if(btn.id == "btnRefresh" ){ 
                 oCarrierExposureShipmentGrid.dataSource.read();
             } else if(btn.id == "btnResetCurrentUserConfig"){ resetCurrentUserConfig(PageControl); }
         }
         var blnCarrierExposureShipmentGridChangeBound = false;
         //*************  Call Back Functions ****************
         function CarrierExposureShipmentGridDataBoundCallBack(e,tGrid){           
             oCarrierExposureShipmentGrid = tGrid;
             if (blnCarrierExposureShipmentGridChangeBound == false){
                 oCarrierExposureShipmentGrid.bind("change", saveCarrierExposureShipmentPK);
                 blnCarrierExposureShipmentGridChangeBound = true;
             }
             //if iCarrierExposureShipmentPK is not 0 select that row in the grid
             if (typeof (iCarrierExposureShipmentPK) !== 'undefined' && iCarrierExposureShipmentPK !== null && iCarrierExposureShipmentPK !== 0) {
                 var rows = oCarrierExposureShipmentGrid.items();
                 $(rows).each(function(e) {
                     var row = this;
                     var dataItem = oCarrierExposureShipmentGrid.dataItem(row);
                     if (dataItem.RunTaskControl == iCarrierExposureShipmentPK) { 
                         oCarrierExposureShipmentGrid.select(row); 
                         return false;
                     }
                 });
             }
             ...
             //lblBalanceID =  wdgtCarrierExposureShipmentGridEdit.GetFieldID("lblBalanceTotal");
             //wdgtCarrierExposureShipmentGridEdit.SetFieldDefault("lblBalanceTotal",'$100.00');
         ...
         $(document).ready(function () {
            ...
            if (control != 0){
                ...
                getPageSettings(tPage, "CarrierExposureShipment", "pk", false);
            }
