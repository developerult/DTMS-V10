<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="CarrierExposureAll.aspx.cs" Inherits="DynamicsTMS365.CarrierExposureAll" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head >
    <title>DTMS Carrier Exposure All Shipment Alerts</title>        
         <%=cssReference%>             
        <style>
            html,
    ...
    <% Response.WriteFile("~/Views/HelpWindow.html"); %>   
     ...
     <script>
        ...
        var PageControl = '<%=PageControl%>'; 
         var oCarrierExposureAllGrid = null;
         var wnd = kendo.ui.Window; 
         var tObj = this;
         var tPage = this;
         var iCarrierExposureAllPK = 0;
        ...
        <% Response.Write(NGLOAuth2); %>
                             ...
                             if (data.Data.length > 0 && typeof (data.Data[0]) !== 'undefined' && ngl.isObject(data.Data[0])) {
                                 ...
                                 if(typeof (userPageSettings) !== 'undefined' && userPageSettings != null && userPageSettings.value != undefined){
                                     if (userPageSettings.name === "pk"){ 
                                         if(typeof (iCarrierExposureAllPK) === 'undefined' || iCarrierExposureAllPK == null || iCarrierExposureAllPK === 0){ iCarrierExposureAllPK = userPageSettings.value; }                                 
                                     }                                                                                                                                  
                                 }                                   
                             }                              
                         }                            
         ...
         function saveCarrierExposureAllPK() {
             try {
                 var row = oCarrierExposureAllGrid.select();
                 ...
                 var dataItem = oCarrierExposureAllGrid.dataItem(row);
                 ...
                 if ("RunTaskControl" in dataItem){
                     ...
                     iCarrierExposureAllPK = dataItem.RunTaskControl;
                     var setting = {name:'pk', value: iCarrierExposureAllPK.toString()};
                     var oCRUDCtrl = new nglRESTCRUDCtrl();
                     var blnRet = oCRUDCtrl.update("CarrierExposureAll/PostPageSetting", setting, tPage, "savePostPageSettingSuccessCallback", "savePostPageSettingSuccessCallbackAjaxErrorCallback",tPage);
                     return true;
                 } else {
                     ngl.showValidationMsg("TMS Task Required", "Please select a TMS Task Record to continue", tPage);
                     return false;
         ...
         function isCarrierExposureAllSelected() {
             if (typeof (iCarrierExposureAllPK) === 'undefined' || iCarrierExposureAllPK === null || iCarrierExposureAllPK === 0) {
                 return saveCarrierExposureAllPK();
             }
             return true;
         }
         function execActionClick(btn, proc){            
             if(btn.id == "btnRefresh" ){ 
                 oCarrierExposureAllGrid.dataSource.read();
             } else if(btn.id == "btnResetCurrentUserConfig"){ resetCurrentUserConfig(PageControl); }
         }
         var blnCarrierExposureAllGridChangeBound = false;
         //*************  Call Back Functions ****************
         function CarrierExposureAllGridDataBoundCallBack(e,tGrid){           
             oCarrierExposureAllGrid = tGrid;
             if (blnCarrierExposureAllGridChangeBound == false){
                 oCarrierExposureAllGrid.bind("change", saveCarrierExposureAllPK);
                 blnCarrierExposureAllGridChangeBound = true;
             }
             //if iCarrierExposureAllPK is not 0 select that row in the grid
             if (typeof (iCarrierExposureAllPK) !== 'undefined' && iCarrierExposureAllPK !== null && iCarrierExposureAllPK !== 0) {
                 var rows = oCarrierExposureAllGrid.items();
                 $(rows).each(function(e) {
                     var row = this;
                     var dataItem = oCarrierExposureAllGrid.dataItem(row);
                     if (dataItem.RunTaskControl == iCarrierExposureAllPK) { 
                         oCarrierExposureAllGrid.select(row); 
                         return false;
                     }
                 });
             }
             ...
             //lblBalanceID =  wdgtCarrierExposureAllGridEdit.GetFieldID("lblBalanceTotal");
             //wdgtCarrierExposureAllGridEdit.SetFieldDefault("lblBalanceTotal",'$100.00');
         ...
         $(document).ready(function () {
            ...
            if (control != 0){
                ...
                getPageSettings(tPage, "CarrierExposureAll", "pk", false);
            }
