<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="LocalReports.aspx.cs" Inherits="DynamicsTMS365.LocalReports" %>

<!DOCTYPE html>

<html>
    <head >
        <title>DTMS Local Reports</title>         
         <%=cssReference%>             
        <style>
            html,
    ...
    <script>
        ...
        function renderReport(name, control) {
            var reportsServerUrl = '<%=ConfigurationManager.AppSettings["ReportsServerUrl"] %>';
            var reportsStdPath = '<%=ConfigurationManager.AppSettings["ReportsStdPath"] %>';
            var reportsCustomPath = '<%=ConfigurationManager.AppSettings["ReportsCustomPath"] %>';
            if (control >= 1000)
            {
                //custom reports
                path = "FMCustomReports";
                var url = reportsServerUrl + "Pages/ReportViewer.aspx?%2f" + reportsCustomPath + "%2f" + name + "&rs:Command=Render";
                openInNewTab(url);
            }
            else
            {
                //standard reports
                path = "FMStdReports";
                var url = reportsServerUrl + "Pages/ReportViewer.aspx?%2f" + reportsStdPath + "%2f" + name + "&rs:Command=Render&UserName=" + localStorage.NGLvar1455;
                openInNewTab(url);
            }           
        }
