...
function kendoMasks() {
    ...
    this.loadDefaultMasks = function () {
        //Modified by RHR for v-8.5.4.005 on 04/05/2024
...
function DispatchingDialogCtrl() {
    ...
    this.edit = function (data) {
        ...
        if (typeof (data) !== 'undefined' && ngl.isObject(data)) {
            this.updateLoadSummaryData();
            console.log(data);
            //console.log(data.Items[0].oBookAccessorial[0].BookAcssValue);
            const totalAccessorial = data.Items[0].oBookAccessorial?.reduce(
                (sum, acc) => sum + (acc.BookAcssValue || 0),
                0
            );
            //load all the data from the Dispatch Object to the Screen object
            $("#spProviderSCAC").html(data.ProviderSCAC);
            ...
            $("#spLineHaul").html(ngl.currencyFormat(data.LineHaul));
            $("#spFuel").html(ngl.currencyFormat(data.Fuel));
            $("#spFees").html(ngl.currencyFormat(totalAccessorial));
            var total = (parseFloat(data.TotalCost) || 0) + (parseFloat(totalAccessorial) || 0);
            $("#spTotalCost").html(ngl.currencyFormat(total));
            $("#txtDispOrigName").data("kendoMaskedTextBox").value(data.Origin.Name);
            $("#txtDispOrigContName").data("kendoMaskedTextBox").value(data.Origin.Contact.ContactName);
            $("#txtDispOrigContPhone").data("kendoMaskedTextBox").value(data.Origin.Contact.ContactPhone);
            $("#txtDispOrigContEmail").data("kendoMaskedTextBox").value(data.Origin.Contact.ContactEmail);
...
function DispatchingReportCtrl() {
    ...
    this.updateHeaderData = function (hdr) {
        console.log(hdr);
        //const totalAccessorial = (hdr.Accessorials || [])
        //    .map(a => parseFloat(a.replace(/[^0-9.-]+/g, ""))) // remove text and '$'
        //    .filter(v => !isNaN(v)) // remove NaN
        //    .reduce((sum, val) => sum + val, 0);

        const totalAccessorial = hdr.Fees;
        console.log(hdr.Fees);

        var total = (parseFloat(hdr.TotalCost) || 0) + (parseFloat(totalAccessorial) || 0);
        ...
        $("#spDispRptFees").html(ngl.currencyFormat(totalAccessorial));
        $("#spDispRptTotalCost").html(ngl.currencyFormat(total));
...
function LaneEAWndCtrl() {
    ...
    dsTimeZone: kendo.data.DataSource;
    ...
    this.getLETimeZoneData = function () {
        
        this.dsTimeZone = new kendo.data.DataSource({
            transport: {
                read: {
                    url: "api/Lane/GetTimeZones/0",
                    headers: { "Authorization": localStorage.NGLvar1454, "USC": localStorage.NGLvar1452 }
                },
                parameterMap: function (options, operation) { return options; }
            },
            ...
            error: function (xhr, textStatus, error) { ngl.showErrMsg("Get Time Zones  JSON Response Error", formatAjaxJSONResponsMsgs(xhr, textStatus, error, "Read Data Failuire"), null); this.cancelChanges(); },
            serverPaging: false,
            serverSorting: false,
            serverFiltering: false
        });

        $("#ddlEALaneOrigTimeZone").data("kendoDropDownList").setDataSource(this.dsTimeZone);
        $("#ddlEALaneDestTimeZone").data("kendoDropDownList").setDataSource(this.dsTimeZone);
    }

    this.refreshLookupLists = function () {
        this.getLECompData();
        this.getLETimeZoneData();
    ...
    this.save = function () {
        ...
        var diOrigTS = $("#ddlEALaneOrigTimeZone").data("kendoDropDownList").dataItem();
        var diDestTS = $("#ddlEALaneDestTimeZone").data("kendoDropDownList").dataItem();
        ...
        if (this.blnIsEdit) {
            ...
            console.log(this.data.LaneOriginAddressUse)
        ...
        if ($("#ddlEALTTransType")) {
            ...
            if (diDir.Control === 3) {
                ...
                if ($('#chkEALaneOrigAddressUse').is(":checked")) {
                    this.data.LaneOriginAddressUse = true;
                    console.log("If in");
                } else {
        ...
        if (typeof (diOrigTS) !== 'undefined' && typeof (diOrigTS.value) !== 'undefined') {
            this.data.LaneOrigTimeZone = diOrigTS.value;
        }

        if (typeof (diDestTS) !== 'undefined' && typeof (diDestTS.value) !== 'undefined') {
            this.data.LaneDestTimeZone = diDestTS.value;
        } 

        this.data.LaneName = $("#txtEALaneName").data("kendoMaskedTextBox").value();
        this.data.LaneNumber = $("#txtEALSystemLaneNumber").data("kendoMaskedTextBox").value();
        this.data.ReferenceLaneNumber = $("#txtEALaneNumber").data("kendoMaskedTextBox").value();
    ...
    this.edit = function (data) {
        ...
        if (typeof (data) !== 'undefined' && data != null && ngl.isObject(data)) {
            ...
            if ('LaneControl' in data) {
                ...
                if (typeof (data.LaneOrigTimeZone) !== 'undefined' && ngl.isNullOrWhitespace(data.LaneOrigTimeZone) === false) {
                    sTimezone = data.LaneOrigTimeZone;
                }

                $('#ddlEALaneOrigTimeZone').data('kendoDropDownList').search(sTimezone);

                if (typeof (data.LaneDestTimeZone) !== 'undefined' && ngl.isNullOrWhitespace(data.LaneDestTimeZone) === false) {
                    sTimezone = data.LaneDestTimeZone;
                } else {
                    sTimezone = "Central Standard Time";
                }
                $('#ddlEALaneDestTimeZone').data('kendoDropDownList').search(sTimezone);

                console.log("Ref: " + data.ReferenceLaneNumber);
                console.log("LN: " + data.LaneNumber);
                console.log("Element:", $("#txtEALSystemLaneNumber"));
                console.log("Kendo Widget:", $("#txtEALSystemLaneNumber").data("kendoMaskedTextBox"));
                const laneBox = $("#txtEALSystemLaneNumber").data("kendoMaskedTextBox");
                ...
                if (laneBox && 'LaneName' in data) { $("#txtEALaneName").data("kendoMaskedTextBox").value(data.LaneName); }
                if ('ReferenceLaneNumber' in data) { $("#txtEALaneNumber").data("kendoMaskedTextBox").value(data.ReferenceLaneNumber); }
                if ('LaneNumber' in data) { $("#txtEALSystemLaneNumber").data("kendoMaskedTextBox").value(data.LaneNumber); }
    ...
    this.close = function (e) {
        ...
        $("#txtEALSystemLaneNumber").data("kendoMaskedTextBox").value("");
        ...
        $("#txtEALSystemLaneNumber").data("kendoMaskedTextBox").readonly(true);
        //reset the LE so we get it fresh from the screen everytime
        this.screenLEName = "";
        screenLEControl = 0;
    ...
    this.loadDefaults = function (pageVariable, saveCallBack, deleteCallBack) {
        ...
        if (typeof (pageVariable) !== 'undefined' && ngl.isObject(pageVariable)) {
            ...
            $("#txtEALSystemLaneNumber").kendoMaskedTextBox();
            ...
            $("#ddlEALaneOrigTimeZone").kendoDropDownList({
                autoBind: true,
                dataTextField: "value",
                dataValueField: "value",
                dataSource: this.dsTimeZone,
                autoWidth: true,
                optionLabel: {
            ...
            $("#ddlEALaneDestTimeZone").kendoDropDownList({
                autoBind: true,
                dataTextField: "value",
                dataValueField: "value",
                dataSource: this.dsTimeZone,
