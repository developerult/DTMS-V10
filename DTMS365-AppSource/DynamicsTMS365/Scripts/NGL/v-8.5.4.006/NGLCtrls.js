...
function NGLPopupWindCtrl() {
            ...
            $.each(this.dataFields, function (index, item) {
                ...
                if (tObj.blnEditing === true && ((item.fieldVisible === false && item.fieldReadOnly === true) || (item.fieldVisible === false && (item.fieldRequired === false || item.fieldInsertOnly === true)))) {
                    ...
                    if (item.fieldGroupSubType === nglGroupSubTypeEnum.NGLWorkFlowOptionCtrl) {
                        ...
                        // modified by RHR for v-8.5.4.004 KendoR32023 we must always use the kendo button not a span
                        if (isEmpty(item.fieldStyle)) {
                                item.fieldStyle = 'k-icon k-i-connector'; //set as default
                        }
                        sFormated = kendo.format("&nbsp;&nbsp;<button id='btn{2}' class='{0}'>{1}</button>&nbsp;&nbsp;", item.fieldCssClass, item.fieldCaption, item.fieldTagID);
        ...
        if (typeof (this.dataFields) !== 'undefined' && ngl.isObject(this.dataFields) === true && ngl.isArray(this.dataFields) === true) {
            ...
            $.each(this.dataFields, function (index, item) {
                ...
                if (tObj.blnEditing === true && ((item.fieldVisible === false && item.fieldReadOnly === true) || (item.fieldVisible === false && (item.fieldRequired === false || item.fieldInsertOnly === true)))) {
                    ...
                    //Modified by RHR for v-8.5.4.004 on 12/27/2023 fixed issue with kendoButton logic
                    var nglkendoitem = $("#btn" + item.fieldTagID).data('kendoButton');
                    if (!nglkendoitem) {                        
                        $("#btn" + item.fieldTagID).kendoButton({
                            iconClass: item.fieldStyle,
                            click: function (e) {
                                tPage['' + tObj.sNGLCtrlName + ''].raiseAction('' + item.fieldWidgetAction + '');
                            }
                        });
                        nglkendoitem = $("#btn" + item.fieldTagID).data('kendoButton');
                    }                    
    ...
    this.saveSuccessCallback = function (data) {
        ...
        kendo.ui.progress(nglkendoWindow.element, false);
    ...
    this.save = function () {
        ...
        //alert('saving RateIt');
        var nglkendoWindow = $("#" + this.ContainerDivID).data("kendoWindow");
        kendo.ui.progress(nglkendoWindow.element, true);
        var tObj = this;
                    ...
                    if (typeof (elmt) !== 'undefined' && elmt != null) {
                        try {
                            if (
                                ...
                                field = ngl.convertDateTimeToDateString(elmt.data("kendoDatePicker").value(), null, null);
                                ...
                                //  removes the Location Key so time does not change when sent to server.
                                field = ngl.convertDateTimeToDateString(elmt.data("kendoDateTimePicker").value(), null, null);
    ...
    this.show = function (fk) {
        ...
        if (!nglkendoWindow) {
            ...
            try {
                ...
                try {
                    $("#" + this.ContainerDivID).data("kendoWindow").wrapper.find(".k-svg-i-save").parent().click(function (e) { e.preventDefault(); tObj.save(); });
                } catch (err) {
        ...
        if (this.ctrlContainers != null) {
            $.each(this.ctrlContainers, function (index, item) {
                ...
                if (item.fieldGroupSubType === nglGroupSubTypeEnum.NGLWorkFlowOptionCtrl) {
                    ...
                    //get a reference to the fast tab's grid using the CRUD Tag ID
                    //alert("read datasource for grid: " + item.fieldTagID);
                ...
                $.each(tObj.dataFields, function (index, nitem) {
                    if (nitem.fieldTagID != sID) {
                        if (nitem.fieldGroupSubType == nglGroupSubTypeEnum.NGLWorkFlowOnOffSwitch
                            ...
                            $("#" + nitem.fieldTagID).data("kendoSwitch").check(false);
                            $("#" + nitem.fieldTagID).data("kendoSwitch").enable(false);
                        ...
                        if (nitem.fieldTagID !== sID) {
                            if (nitem.fieldParentTagID == sParentTagID) {
                                ...
                                if (nitem.fieldGroupSubType == nglGroupSubTypeEnum.NGLWorkFlowOnOffSwitch
                                    ||
                                    nitem.fieldGroupSubType == nglGroupSubTypeEnum.NGLWorkFlowYesNoSwitch) {
                                    $("#" + nitem.fieldTagID).data("kendoSwitch").enable(true);
                                    $("#li" + nitem.fieldTagID).show();
                                    var sDefault = 'false';
                                    if ($("#" + nitem.fieldTagID).data("kendoSwitch").check() == true) { sDefault = 'true'; }
                ...
                if (oParentItem.fieldGroupSubType == nglGroupSubTypeEnum.NGLWorkFlowGroup) {
                    ...
                    $.each(tObj.dataFields, function (index, nitem) {
                        if (nitem.fieldTagID !== sID
                            ...
                            if ($("#" + nitem.fieldTagID).data("kendoSwitch").check() == true) {
                            ...
                            if (nitem.fieldParentTagID != sParentTagID) {
                                ...
                                if (nitem.fieldGroupSubType == nglGroupSubTypeEnum.NGLWorkFlowSectionCtrl && blnIsOneGroupItemChecked == false) {
                                    ...
                                    if ($("#div" + nitem.fieldTagID + "wrapper").is(':visible')) {
                                        ...
                                        // Modified by RHR for v-8.5.4.001 on 05/08/2023 we need to use the checked value to determine the Default Value
                                        if (nitem.fieldGroupSubType == nglGroupSubTypeEnum.NGLWorkFlowOnOffSwitch
                                            ||
                                            nitem.fieldGroupSubType == nglGroupSubTypeEnum.NGLWorkFlowYesNoSwitch) {
                                            if ($("#" + nitem.fieldTagID).data("kendoSwitch").check() == true) {
                                                ...
                                                tObj.workFlowSettings.push({ fieldID: nitem.fieldID, fieldName: nitem.fieldName, fieldDefaultValue: 'false', fieldVisible: 'true', fieldReadOnly: "false" })
                                            }

                                        } else {
                                            tObj.workFlowSettings.push({ fieldID: nitem.fieldID, fieldName: nitem.fieldName, fieldDefaultValue: 'false', fieldVisible: 'false', fieldReadOnly: "false" })
                                        }
...
function NGLFastTabCtrl() {
    ...
    this.loadHTML = function () {
        ...
        //divContainer.append(kendo.format("<span id={0} style='{1}'><a onclick=\"expandFastTab({0}, {2}, {3}, {4});\"><span class='k-icon k-i-chevron-down ui-span-container'></span></a></span>",sExtendID,cssDisplayStyle,sCollapseID,sHeaderID,sDetailID));
        //if (this.Expanded === true){cssDisplayStyle = "display: none;";} else {cssDisplayStyle = "display: normal;"; }
        //divContainer.append(kendo.format("<span id={2} style='{1}'><a onclick=\"collapseFastTab({0}, {2}, {3}, {4});\"><span class='k-icon k-i-chevron-up ui-span-container'></span></a></span>",sExtendID,cssDisplayStyle,sCollapseID,sHeaderID,sDetailID));
    ...
    this.show = function () {
        ...
        if (!nglkendoWindow) {
            try {
                ...
                try { $("#" + this.ContainerDivID).data("kendoWindow").wrapper.find(".k-svg-i-save").parent().click(function (e) { e.preventDefault(); tObj.save(); }); }
