...
//platf ?
//username ?
var opostLogoutRedirectUri = window.location.href + 'NGLLogin' ;
var oredirectUri = window.location.href + 'NGLLogin';
var oidaClient = '7a6d49ef-446b-41b2-97ec-fc97ab7309f0' // 'ab8def6a-9066-4657-8118-dcb775e48a92';
var oAuth2instasnce = 'https://login.microsoftonline.com/common/';
...
function validateUser(suppressMsgs, ucRequired, sCaller) {
    ...
    if (ssoaControl === "1") {
        ...
        if (cToken != null) {
            ...
            localStorage.SignedIn = "f";
            if (ucRequired === true) {
                // When after an hour cToken == null this else works.
                document.location = "../NGLLogin.aspx?caller=" + sCaller;
                blnReloadPage = true;
            } else {
                //signIn();
...
function getEpochOf6HoursLater() {
    // Get the current time in milliseconds since January 1, 1970
    var currentTime = new Date().getTime();

    // Calculate the time after 6 hours (6 * 60 * 60 * 1000 milliseconds)
    var sixHoursLater = currentTime + (6 * 60 * 60 * 1000);

    // Return the epoch time of 6 hours later
    return Math.floor(sixHoursLater / 1000); // Convert milliseconds to seconds
}

// Example usage
var epochOf6HoursLater = getEpochOf6HoursLater();

function calculateNewExpiryTime() {
    // Calculate new expiry time, e.g., add a fixed duration to current time
    var currentTime = Math.floor(Date.now() / 1000);
    var newExpiry = currentTime + 3600;
    return newExpiry;
}
