<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="LoadBoardRevenue.aspx.cs" Inherits="DynamicsTMS365.LoadBoardRevenue" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head >
    <title>DTMS Load Board Revenue</title>        
         <%=cssReference%>             
        <style>
            html,
    ...
    <% Response.WriteFile("~/Views/HelpWindow.html"); %>   
     ...
     <script>
        ...
        var PageControl = '<%=PageControl%>'; 
         ...
         var tObj = this;
         var tPage = this;
         var oLBRevFeesGrid = null;
         var oLBRevChargesGrid = null;
         var oLBRevCostsGrid = null;
         var oLBRevSavingsGrid = null;   
        ...
        <% Response.Write(NGLOAuth2); %>
         ...
         function execActionClick(btn, proc) {            
             ...
             else { ngl.pgNavigation(btn.id,true, BookControlKey); } // modified by RHR for v-8.5.4.004 on 12/06/2023 added page navigation menu methodd
         }
         // modified by RHR for v-8.5.2.006 on 12/22/2022 added refresh method
         function refresh() {
             if (oLBRevFeesGrid) { oLBRevFeesGrid.dataSource.read(); }
             if (oLBRevChargesGrid) { oLBRevChargesGrid.dataSource.read(); }
             if (oLBRevCostsGrid) { oLBRevCostsGrid.dataSource.read(); }
             if (oLBRevSavingsGrid) { oLBRevSavingsGrid.dataSource.read(); }
         ...
         var sVariances = ["QvE_Variance","QvB_Variance","EvB_Variance","QvP_Variance","EvP_Variance","BvP_Variance"
             ,"QvAj_Variance","EvAj_Variance","BvAj_Variance","PvAj_Variance","QvA_Variance","EvA_Variance"
             ,"BvA_Variance","PvA_Variance","AjvA_Variance","QvPd_Variance","EvPd_Variance","BvPd_Variance"
             ,"PvPd_Variance","AjvPd_Variance","AvPd_Variance"]
         ...
         function LBRevFeesGridDataBoundCallBack(e,tGrid){           
             oLBRevFeesGrid = tGrid;
         ...
         function LBRevChargesGridDataBoundCallBack(e,tGrid){           
             oLBRevChargesGrid = tGrid;             
             var ds = tGrid.dataSource.data();
             for (var j=0; j < ds.length; j++) {                 
                 var row = e.sender.tbody.find("[data-uid='" + ds[j].uid + "']");
                 addFinancialNumericClassToGridColumns(tGrid,ds,j,row);
         ...
         function LBRevCostsGridDataBoundCallBack(e,tGrid){           
             oLBRevCostsGrid = tGrid;
             var ds = tGrid.dataSource.data();
             for (var j=0; j < ds.length; j++) {                 
                 var row = e.sender.tbody.find("[data-uid='" + ds[j].uid + "']");
                 addFinancialNumericClassToGridColumns(tGrid,ds,j,row);
         ...
         function LBRevSavingsGridDataBoundCallBack(e,tGrid){           
             oLBRevSavingsGrid = tGrid;
         }
