<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="OutdatedNStatusOrders.aspx.cs" Inherits="DynamicsTMS365.OutdatedNStatusOrders" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head >
    <title>DTMS Outdated N Status Order Alerts</title>        
         <%=cssReference%>             
        <style>
            html,
    ...
    <% Response.WriteFile("~/Views/HelpWindow.html"); %>   
     ...
     <script>
        ...
        var PageControl = '<%=PageControl%>'; 
         var oOutdatedNStatusOrdersGrid = null;
         var wnd = kendo.ui.Window; 
         var tObj = this;
         var tPage = this;
         var iOutdatedNStatusOrdersPK = 0;
        ...
        <% Response.Write(NGLOAuth2); %>
                             ...
                             if (data.Data.length > 0 && typeof (data.Data[0]) !== 'undefined' && ngl.isObject(data.Data[0])) {
                                 ...
                                 if(typeof (userPageSettings) !== 'undefined' && userPageSettings != null && userPageSettings.value != undefined){
                                     if (userPageSettings.name === "pk"){ 
                                         if(typeof (iOutdatedNStatusOrdersPK) === 'undefined' || iOutdatedNStatusOrdersPK == null || iOutdatedNStatusOrdersPK === 0){ iOutdatedNStatusOrdersPK = userPageSettings.value; }                                 
                                     }                                                                                                                                  
                                 }                                   
                             }                              
                         }                            
         ...
         function saveOutdatedNStatusOrdersPK() {
             try {
                 var row = oOutdatedNStatusOrdersGrid.select();
                 ...
                 var dataItem = oOutdatedNStatusOrdersGrid.dataItem(row);
                 ...
                 if ("RunTaskControl" in dataItem){
                     ...
                     iOutdatedNStatusOrdersPK = dataItem.RunTaskControl;
                     var setting = {name:'pk', value: iOutdatedNStatusOrdersPK.toString()};
                     var oCRUDCtrl = new nglRESTCRUDCtrl();
                     var blnRet = oCRUDCtrl.update("OutdatedNStatusOrders/PostPageSetting", setting, tPage, "savePostPageSettingSuccessCallback", "savePostPageSettingSuccessCallbackAjaxErrorCallback",tPage);
                     return true;
                 } else {
                     ngl.showValidationMsg("TMS Task Required", "Please select a TMS Task Record to continue", tPage);
                     return false;
         ...
         function isOutdatedNStatusOrdersSelected() {
             if (typeof (iOutdatedNStatusOrdersPK) === 'undefined' || iOutdatedNStatusOrdersPK === null || iOutdatedNStatusOrdersPK === 0) {
                 return saveOutdatedNStatusOrdersPK();
             }
             return true;
         }
         function execActionClick(btn, proc){            
             if(btn.id == "btnRefresh" ){ 
                 oOutdatedNStatusOrdersGrid.dataSource.read();
             } else if(btn.id == "btnResetCurrentUserConfig"){ resetCurrentUserConfig(PageControl); }
         }
         var blnOutdatedNStatusOrdersGridChangeBound = false;
         //*************  Call Back Functions ****************
         function OutdatedNStatusOrdersGridDataBoundCallBack(e,tGrid){           
             oOutdatedNStatusOrdersGrid = tGrid;
             if (blnOutdatedNStatusOrdersGridChangeBound == false){
                 oOutdatedNStatusOrdersGrid.bind("change", saveOutdatedNStatusOrdersPK);
                 blnOutdatedNStatusOrdersGridChangeBound = true;
             }
             //if iOutdatedNStatusOrdersPK is not 0 select that row in the grid
             if (typeof (iOutdatedNStatusOrdersPK) !== 'undefined' && iOutdatedNStatusOrdersPK !== null && iOutdatedNStatusOrdersPK !== 0) {
                 var rows = oOutdatedNStatusOrdersGrid.items();
                 $(rows).each(function(e) {
                     var row = this;
                     var dataItem = oOutdatedNStatusOrdersGrid.dataItem(row);
                     if (dataItem.RunTaskControl == iOutdatedNStatusOrdersPK) { 
                         oOutdatedNStatusOrdersGrid.select(row); 
                         return false;
                     }
                 });
             }
             ...
             //lblBalanceID =  wdgtOutdatedNStatusOrdersGridEdit.GetFieldID("lblBalanceTotal");
             //wdgtOutdatedNStatusOrdersGridEdit.SetFieldDefault("lblBalanceTotal",'$100.00');
         ...
         $(document).ready(function () {
            ...
            if (control != 0){
                ...
                getPageSettings(tPage, "OutdatedNStatusOrders", "pk", false);
            }
