<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="CarrierFuel.aspx.cs" Inherits="DynamicsTMS365.CarrierFuel" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head >
    <title>DTMS Carrier Fuel</title>        
         <%=cssReference%>             
        <style>
            html,
...
<body>
                ...
                <div id="top-pane">
                  <div id="horizontal" style="height: 100%; width: 100%; ">
                        ...
                        <div id="center-pane">                            
                            ...
                            <div id="lblMatrixInUse" style="text-align:center;"><p>Using Carrier Specific Fuel Matrix</p></div>
    ...
    <% Response.WriteFile("~/Views/CarrierFuelMatrixWindow.html"); %>
     ...
     <script>
        ...
        <% Response.Write(NGLOAuth2); %>
         ...
         var oCarrierFuelAdRateGrid = null;         
         var oCarrierFuelAdExGrid = null;

         function execBeforeCarrierFuelGridInsert(){
         ...
         function execActionClick(btn, proc) {
             //debugger;
             if (btn.id == "btnUpdateAllActiveLoads"){
             ...
             else if(btn.id == "btnImportCarSpecificFuelMatrix" ){ openImportCarFuelWnd(PageControl); }
             else if(btn === "Saved"){ 
                 if (typeof (tPage[proc]) !== 'undefined' && ngl.isObject(tPage[proc])) {
                     if (tPage[proc].sNGLCtrlName == "wdgtNewFuelAdTemplWndDialog") {
                         
                         var iCarrTarControlElmt = wdgtvCarrierFuelSummarySummary.GetFieldID("CarrierControl");
                         ...
                         oCRUDCtrl.filteredPost("CarrierFuelAddendum/CreateNewFuelAddendumFromTemplate", sSummaryCarrTarControlID, tObj, "saveNewFuelAddendumFromTemplateCallback", "saveNewFuelAddendumFromTemplateAjaxErrorCallback", true) ;
             ...
             //else if (btn.id == "btnUploadTariffFuelMatrix"){ openImportFuelWnd(PageControl);}
         }

         function refresh(){
             //wdgtvCarrierFuelSummarySummary.read(0); 
             //wdgtTariffFuelAddendumEdit.read(0);
             //oTariffFuelAdRateGrid.dataSource.read();
             //oTariffFuelAdExGrid.dataSource.read();

             wdgtvCarrierFuelSummarySummary.read(0); 
             wdgtCarrierFuelAddendumEdit.read(0);
             oCarrierFuelAdRateGrid.dataSource.read();
             oCarrierFuelAdExGrid.dataSource.read();
         }

         function refreshFuelAdRateGrid(){
             //wdgtvCarrierFuelSummarySummary.read(0); 
             //wdgtTariffFuelAddendumEdit.read(0);
             //oTariffFuelAdRateGrid.dataSource.read();
             //oTariffFuelAdExGrid.dataSource.read();
         ...
         function CarrierFuelAdRateGridDataBoundCallBack(e,tGrid){           
             oCarrierFuelAdRateGrid = tGrid;           
         }

         function CarrierFuelAdExGridDataBoundCallBack(e,tGrid){           
             ocarrierFuelAdExGrid = tGrid;
         }

         function saveNewFuelAddendumFromTemplateCallback(data) {
             // alert("Fuel Template Saved Callback");
         ...
         //                oResults.error.name = "Create Carrier fuel addendum from template Failure";
         ...
         //                    blnErrorShown = true;
         //                    oResults.error = new Error();
         //                    oResults.error.name = "Unable to create your Carrier fuel addendum";
         ...
         function UpdateAllActiveLoads(iRet) {
             ...
             if (typeof (wdgtvCarrierFuelSummarySummary) !== 'undefined' && ngl.isObject(wdgtvCarrierFuelSummarySummary)) {
                 var iCarrTarControlElmt = wdgtvCarrierFuelSummarySummary.GetFieldID("CarrierControl");
                 var elmt = $("#" + iCarrTarControlElmt); 
                 var sSummaryCarrTarControlID = "0";
                 if (typeof (elmt) !== 'undefined' && elmt != null){
                 ...
                 var oCRUDCtrl = new nglRESTCRUDCtrl();
                 oCRUDCtrl.filteredPost("CarrierFuelAddendum/UpdateCarrierFuelFeesByContract", sSummaryCarrTarControlID, tObj, "saveCarrierFuelFeesCallback", "saveCarrierFuelFeesAjaxErrorCallback", true) ;
             }
         }
         ...
         function saveCarrierFuelFeesCallback(data){
             ...
             try {
                 ...
                 if (typeof (data) !== 'undefined' && ngl.isObject(data)) {
                     if (typeof (data.Errors) !== 'undefined' && data.Errors != null && data.Errors.length > 0) {
                         oResults.error = new Error();
                         oResults.error.name = "Update All Carrier Fuel Fees From Carrier Failure";
                         oResults.error.message = data.Errors;
                         blnErrorShown = true;
                         ngl.showErrMsg(oResults.error.name, oResults.error.message, null);
                     }
                         ...
                         if (typeof (data.Data) !== 'undefined' && data.Data != null && ngl.isArray(data.Data) && data.Data[0] == true) {
                             ...
                             blnErrorShown = true;
                             oResults.error = new Error();
                             oResults.error.name = "Unable to Update All Carrier Fuel Fees From Carrier Fuel";
                             oResults.error.message = "The procedure returned false, please refresh your data and try again.";
                             ngl.showErrMsg(oResults.error.name, oResults.error.message, null);
                         }
                     }
                 ...
                 if (blnSuccess === false && blnErrorShown === false) {
                     oResults.error = new Error();
                     oResults.error.name = "Update All Carrier Fuel Fees From Carrier Fuel Failure";
                     oResults.error.message = "No results are available.";
                     ngl.showErrMsg(oResults.error.name, oResults.error.message, tObj);
                 }
             } catch (err) { oResults.error = err; ngl.showErrMsg(err.name, err.message, null); }            
         ...
         //            var tObj = tPage;
         //            var oCRUDCtrl = new nglRESTCRUDCtrl();
         //            var blnRet = oCRUDCtrl.delete("CarrierFuelAddendum/DELETE", intCarrFuelAdControl, tObj, "deleteCarSpecificFuelMatrixCallback", "deleteCarSpecificFuelMatrixAjaxErrorCallback")
         //        }
         //    }
         //}
         ...
         function CarrierFuelAddendumCB(oResults){
             if (oResults.source == "readSuccessCallback")
             {
                 if(typeof (oResults) !== 'undefined' && oResults != null){
                     ...
                     if(typeof (oResults.data) !== 'undefined' && oResults.data != null && ngl.isArray(oResults.data) && oResults.data.length > 0 ){ 
                         if (!("CarrFuelAdCarrierControl" in oResults.data[0])){ ngl.showValidationMsg("Tariff Fuel Addendum Data Error", "Data Record is missing the field CarrFuelAdCarrTarControl", tPage); return; }
                     ...
                     //var topWrapperIDAd;
                     //if(typeof (oCarrierFuelAdRateGrid) !== 'undefined' && oCarrierFuelAdRateGrid != null){
                     //    var psAd = oCarrierFuelAdRateGrid.wrapper.parents();                     
                     ...
                     ////oTariffFuelAdExGrid
                     //var topWrapperIDEx;
                     //if(typeof (oCarrierFuelAdExGrid) !== 'undefined' && oCarrierFuelAdExGrid != null){
                     //    var psEx = oCarrierFuelAdExGrid.wrapper.parents();                 
                     ...
                     //var topWrapperIDTFA;
                     //if (wdgtCarrierFuelAddendumEdit) { topWrapperIDTFA = wdgtCarrierFuelAddendumEdit.ContainerDivID; }
                     ...
                     //}
                     //var gridWrapper = oCarrierFuelAdRateGrid.wrapper;
