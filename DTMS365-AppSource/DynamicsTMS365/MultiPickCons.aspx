<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="MultiPickCons.aspx.cs" Inherits="DynamicsTMS365.MultiPickCons" %>
    ...
    <% Response.WriteFile("~/Views/HelpWindow.html"); %>   
     ...
     <script>
        ...
        var PageControl = '<%=PageControl%>'; 
         var oMultiPickConsGrid = null;
         var wnd = kendo.ui.Window; 
         var tObj = this;
         var tPage = this;
         var iMultiPickConsPK = 0;
        ...
        <% Response.Write(NGLOAuth2); %>
                             ...
                             if (data.Data.length > 0 && typeof (data.Data[0]) !== 'undefined' && ngl.isObject(data.Data[0])) {
                                 ...
                                 if(typeof (userPageSettings) !== 'undefined' && userPageSettings != null && userPageSettings.value != undefined){
                                     if (userPageSettings.name === "pk"){ 
                                         if(typeof (iMultiPickConsPK) === 'undefined' || iMultiPickConsPK == null || iMultiPickConsPK === 0){ iMultiPickConsPK = userPageSettings.value; }                                 
                                     }                                                                                                                                  
                                 }                                   
                             }                              
                         }                            
         ...
         function saveMultiPickConsPK() {
             try {
                 var row = oMultiPickConsGrid.select();
                 if (typeof (row) === 'undefined' || row == null) {              
                     ngl.showValidationMsg("Multi-Pick Record Required", "Please select a Multi-Pick Record Record to continue", tPage);
                     return false;
                 }               
                 //Get the dataItem for the selected row
                 var dataItem = oMultiPickConsGrid.dataItem(row);
                 if (typeof (dataItem) === 'undefined' || dataItem == null) {              
                     ngl.showValidationMsg("Multi-Pick Record Required", "Please select a Multi-Pick Record Record to continue", tPage);
                     return false;
                 }  
                 if ("MultiPickBookControl" in dataItem){
                
                     iMultiPickConsPK = dataItem.MultiPickBookControl;
                     var setting = {name:'pk', value: iMultiPickConsPK.toString()};
                     var oCRUDCtrl = new nglRESTCRUDCtrl();
                     var blnRet = oCRUDCtrl.update("MultiPickCons/PostPageSetting", setting, tPage, "savePostPageSettingSuccessCallback", "savePostPageSettingSuccessCallbackAjaxErrorCallback",tPage);
                     return true;
                 } else {
                     ngl.showValidationMsg("Multi-Pick Record Required", "Please select a Multi-Pick Record Record to continue", tPage);
                     return false;
         ...
         function isMultiPickConsSelected() {
             if (typeof (iMultiPickConsPK) === 'undefined' || iMultiPickConsPK === null || iMultiPickConsPK === 0) {
                 return saveMultiPickConsPK();
             }
             return true;
         }
         function execActionClick(btn, proc){            
             if(btn.id == "btnRefresh" ){ 
                 oMultiPickConsGrid.dataSource.read();
             } else if(btn.id == "btnResetCurrentUserConfig"){ resetCurrentUserConfig(PageControl); }
         }
         var blnMultiPickConsGridChangeBound = false;
         //*************  Call Back Functions ****************
         function MultiPickConsGridDataBoundCallBack(e,tGrid){           
             oMultiPickConsGrid = tGrid;
             if (blnMultiPickConsGridChangeBound == false){
                 oMultiPickConsGrid.bind("change", saveMultiPickConsPK);
                 blnMultiPickConsGridChangeBound = true;
             }
             //if iMultiPickConsPK is not 0 select that row in the grid
             if (typeof (iMultiPickConsPK) !== 'undefined' && iMultiPickConsPK !== null && iMultiPickConsPK !== 0) {
                 var rows = oMultiPickConsGrid.items();
                 $(rows).each(function(e) {
                     var row = this;
                     var dataItem = oMultiPickConsGrid.dataItem(row);
                     if (dataItem.RunTaskControl == iMultiPickConsPK) { 
                         oMultiPickConsGrid.select(row); 
                         return false;
                     }
                 });
             }
             ...
             //lblBalanceID =  wdgtMultiPickConsGridEdit.GetFieldID("lblBalanceTotal");
             //wdgtMultiPickConsGridEdit.SetFieldDefault("lblBalanceTotal",'$100.00');
         ...
         $(document).ready(function () {
            ...
            if (control != 0){
                ...
                getPageSettings(tPage, "MultiPickCons", "pk", false);
            }
