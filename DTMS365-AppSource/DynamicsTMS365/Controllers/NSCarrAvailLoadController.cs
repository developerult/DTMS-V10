...
{
    public class NSCarrAvailLoadController : NGLControllerBase
    {
        #region " Properties"
        /// <summary>
        /// This property is used for logging and error tracking.
        /// </summary>
        private string _SourceClass = "DynamicsTMS365.Controllers.NSCarrAvailLoadController";
        ...
        public Models.Response GetRecords(string filter)
        {
            ...
            return GetvNSCarrAvailableLoads(filter);
        }
        ...
        /// <returns></returns>
        public Models.Response GetvNSCarrAvailableLoads(string filter)
        {
            var response = new Models.Response(); //new HttpResponseMessage();
            if (!authenticateController(ref response)) { return response; }
            ...
            try
            {
                ...
                LTS.vNSAvailablePendingLoad[] ltsView = dalLTData.GetvNSCarrAvailableLoads(ref RecordCount, Parameters.UserCarrierControl, f);

                if (ltsView != null && ltsView.Count() > 0)
                {
                    //RecordCount contains the number of records in the database that matches the filters
                    count = RecordCount;
                    retLanes = (from e in ltsView
