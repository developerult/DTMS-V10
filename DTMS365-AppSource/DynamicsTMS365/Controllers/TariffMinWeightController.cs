...
{
    public class TariffMinWeightController : NGLControllerBase
    {
        ...
        /// </summary>
        public TariffMinWeightController()
        ...
        /// </summary>
        private string _SourceClass = "DynamicsTMS365.Controllers.TariffMinWeightController";
        ...
        public Models.Response Get(int id)
        {
            ...
            try
            {
                ...
                if (id != 0)
                {
                    f.filterName = "CarrTarMinWeightControl";
                    f.filterValue = id.ToString();
                }
                //get the parent control
                f.ParentControl = readPagePrimaryKey(Parameters, Utilities.PageEnum.Tariff);
                DAL.NGLCarrTarMinWeightData oDAL = new DAL.NGLCarrTarMinWeightData(Parameters);
                LTS.vCarrierTariffMinWeight[] oData = new LTS.vCarrierTariffMinWeight[] { };
                oData = oDAL.GetCarrierTariffMinWeights(f, ref RecordCount);
                count = oData.Count();
        ...
        public Models.Response GetAllRecords(string filter)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(filter)) { savePageFilters(filter,"TariffMinWeightFilte"); }
                f.ParentControl = readPagePrimaryKey(Parameters, Utilities.PageEnum.Tariff);
        ...
        public Models.Response Post([System.Web.Http.FromBody]LTS.vCarrierTariffMinWeight data)
        {
            //create a response message to send back
            var response = new Models.Response(); //new HttpResponseMessage();
            if (!authenticateController(ref response)) { return response; }
            ...
            {
                DAL.NGLCarrTarMinWeightData oDAL = new DAL.NGLCarrTarMinWeightData(Parameters);
                bool blnRet = oDAL.SaveCarrierTariffMinWeight(data);
        ...
        public Models.Response DELETE(int id)
        {
            ...
            {
                DAL.NGLCarrTarMinWeightData oDAL = new DAL.NGLCarrTarMinWeightData(Parameters);
                bool blnRet = oDAL.DeleteCarrierTariffMinWeight(id);
                bool[] oRecords = new bool[1];
                oRecords[0] = blnRet;
                response = new Models.Response(oRecords, 1);
            }
