...
{
    public class ManageScheduleLockoutController : NGLControllerBase
    {
        #region " Constructors "

        /// <summary>Initializes the Page property by calling the base class constructor</summary>
        public ManageScheduleLockoutController() : base(Utilities.PageEnum.ManageSchedule) { }
        ...
        private string _SourceClass = "DynamicsTMS365.Controllers.ManageScheduleLockoutController";
        ...
        public Models.Response GetAllRecords(string filter)
        {
            ...
            try
            {
                //if (!string.IsNullOrWhiteSpace(filter)) { savePageFilters(filter, "lockoutGridFilter"); } //save the page filter for the next time the page loads
                DAL.Models.AllFilters f = new System.Web.Script.Serialization.JavaScriptSerializer().Deserialize<DAL.Models.AllFilters>(filter);

                LTS.vAMSApptOverrideLogFailedAttempt[] oData = new LTS.vAMSApptOverrideLogFailedAttempt[] { };
                int RecordCount = 0;
                int count = 0;
                oData = NGLAMSAppointmentData.GetvAMSApptOverrideLogFailedAttempts(ref RecordCount, f);
                if (oData?.Count() > 0)
                {
                    count = oData.Count();
        ...
        public Models.Response Post([FromBody]LTS.vAMSApptOverrideLogFailedAttempt data)
        {
            var response = new Models.Response(); //new HttpResponseMessage();
            if (!authenticateController(ref response)) { return response; }
            try
            {
                ...
                //bool blnRet = NGLAMSAppointmentData.ResetAMSOverrideLogFailedAttempts(data);
        ...
        public Models.Response DELETE(int id)
        {
            ...
            try
            {
                //bool blnRet = NGLAMSAppointmentData.ResetAMSOverrideLogFailedAttempts(id);
                //bool[] oRecords = new bool[1] { blnRet };
                //response = new Models.Response(oRecords, 1);
            }
            catch (Exception ex)
        ...
        [HttpPost, ActionName("ResetFailedAttempt")]
        public Models.Response ResetFailedAttempt([FromBody]LTS.vAMSApptOverrideLogFailedAttempt data)
        {
            var response = new Models.Response(); //new HttpResponseMessage();
            if (!authenticateController(ref response)) { return response; }
            try
