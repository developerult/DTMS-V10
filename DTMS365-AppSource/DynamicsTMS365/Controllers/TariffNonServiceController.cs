...
{
    public class TariffNonServiceController : NGLControllerBase
    {
        #region " Properties"
        /// <summary>
        /// This property is used for logging and error tracking.
        /// </summary>
        private string _SourceClass = "DynamicsTMS365.Controllers.TariffNonServiceController";
        ...
        public Models.Response Get(int id)
        {
            ...
            try
            {
                ...
                if (id != 0)
                {
                    f.filterName = "CarrTarNonServControl";
                    f.filterValue = id.ToString();
                }
                //get the parent control
                f.ParentControl = readPagePrimaryKey(Parameters, Utilities.PageEnum.Tariff);
                DAL.NGLCarrTarNonServData oDAL = new DAL.NGLCarrTarNonServData(Parameters);
                LTS.vCarrierTariffNonService[] oData = new LTS.vCarrierTariffNonService[] { };
                oData = oDAL.GetCarrierTariffNonServices(f, ref RecordCount);
                count = oData.Count();
        ...
        public Models.Response Post([System.Web.Http.FromBody]LTS.vCarrierTariffNonService data)
        {
            //create a response message to send back
            var response = new Models.Response(); //new HttpResponseMessage();
            if (!authenticateController(ref response)) { return response; }
            ...
            {
                DAL.NGLCarrTarNonServData oDAL = new DAL.NGLCarrTarNonServData(Parameters);
                bool blnRet = oDAL.SaveCarrierTariffNonService(data);
        ...
        public Models.Response DELETE(int id)
        {
            ...
            {
                DAL.NGLCarrTarNonServData oDAL = new DAL.NGLCarrTarNonServData(Parameters);
                bool blnRet = oDAL.DeleteCarrierTariffNonService(id);
                bool[] oRecords = new bool[1];
                oRecords[0] = blnRet;
                response = new Models.Response(oRecords, 1);
            }
