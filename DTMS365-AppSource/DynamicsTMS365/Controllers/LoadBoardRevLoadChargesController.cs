...
{
    public class LoadBoardRevLoadChargesController : NGLControllerBase
    {

        #region " Constructors "

        /// <summary>Initializes the Page property by calling the base class constructor</summary>
        public LoadBoardRevLoadChargesController() : base(Utilities.PageEnum.LoadBoardRevenueLoad) { }
        ...
        private string _SourceClass = "DynamicsTMS365.Controllers.LoadBoardRevLoadChargesController";
        ...
        {
            //filters are currently ignored for charges
            var response = new Models.Response(); //new HttpResponseMessage();
            ...
            try
            {
                ...
                //if (!string.IsNullOrWhiteSpace(filter)) { savePageFilters(filter, "LBRevLdChargesFltr"); } //save the page filter for the next time the page loads
                ...
                int iBookControl = 0;
                DModel.vCMLoadBoardRevTemplate[] oData = new DModel.vCMLoadBoardRevTemplate[] { };
                iBookControl = readBookControlPageSetting(iBookControl);
                if (iBookControl == 0)
                {
                    return new Models.Response(oData, 0);
                }

                DAL.NGLBookRevenueData oDAL = new DAL.NGLBookRevenueData(Parameters);                
                oData = oDAL.GetLoadBoardRevLoadCharges(iBookControl);
                count = oData.Count();
