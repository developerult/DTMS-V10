...
{
    public class TariffServiceController : NGLControllerBase
    {
        ...
        /// </summary>
        public TariffServiceController()
                : base(Utilities.PageEnum.TariffServices)
	     {
        }
        ...
        /// </summary>
        private string _SourceClass = "DynamicsTMS365.Controllers.TariffServiceController";
        ...
        public Models.Response Get(int id)
        {
            ...
            try
            {
                ...
                if (id != 0)
                {
                    f.filterName = "CarrTarEquipControl";
                ...
                Models.vCarrierTariffService[] records = new Models.vCarrierTariffService[] { };
                LTS.vCarrierTariffService[] oData = new LTS.vCarrierTariffService[] { };
                oData = oDAL.GetCarrierTariffServices(f, ref RecordCount);
        ...
        public Models.Response Post([System.Web.Http.FromBody]Models.vCarrierTariffService data)
        {
            //create a response message to send back
            var response = new Models.Response(); //new HttpResponseMessage();
            if (!authenticateController(ref response)) { return response; }
            try
            {
                if (data.CarrTarEquipCarrTarControl == 0) {  data.CarrTarEquipCarrTarControl = readPagePrimaryKey(Parameters, Utilities.PageEnum.Tariff); }
                DAL.NGLCarrTarEquipData oDAL = new DAL.NGLCarrTarEquipData(Parameters);
                LTS.vCarrierTariffService oChanges = selectLTSData(data);
                
                bool blnRet = oDAL.SaveCarrierTariffService(oChanges);
