...
{
    public class LEUserProcedureController : NGLControllerBase
    {
        #region " Constructors "
        ...
        /// </summary>
        public LEUserProcedureController()
        ...
        /// </summary>
        private string _SourceClass = "DynamicsTMS365.Controllers.LEUserProcedureController";
        ...
        private Models.Procedure selectModelData(LTS.vRestrictedProceduresByGroup d)
        {
            Models.Procedure modelRecord = new Models.Procedure();
            if (d != null)
            {
                List<string> skipObjs = new List<string> { "" };
                string sMsg = "";
                modelRecord = (Models.Procedure)DTran.CopyMatchingFields(modelRecord, d, skipObjs, ref sMsg);
                //if (modelRecord != null) { modelRecord.setUpdated(d.UserGroupsUpdated.ToArray()); }
            }
            return modelRecord;
        }

        private Models.Procedure selectModelData(DTO.tblProcedureList d)
        {
            Models.Procedure modelRecord = new Models.Procedure();
            if (d != null)
            {
        ...
        public Models.Response GetByParent(int id)
        {
            ...
            try
            {
                ...
                string msg = "";
                savePageSetting(id.ToString(), ref msg, "ProcedureParentRoleControl");

                Models.Procedure[] records = new Models.Procedure[] { };
                int RecordCount = 0;
                int count = 0;
                DAL.Models.AllFilters f = new DAL.Models.AllFilters { ParentControl = id };
                LTS.vRestrictedProceduresByGroup[] oData = NGLUserGroupData.GetRestrictedProceduresByGroup(ref RecordCount, f);
                if (oData?.Count() > 0)
                {
                    count = oData.Count();
        ...
        [HttpGet, ActionName("GetUnRestrictedProceduresByGroup")]
        public Models.Response GetUnRestrictedProceduresByGroup()
        {
            var response = new Models.Response(); //new HttpResponseMessage();
            if (!authenticateController(ref response)) { return response; }
            try
            {
                ...
                DTO.tblProcedureList[] res = NGLSecurityData.GetUnRestrictedProceduresByGroup(groupControl);
                Models.vLookupList[] newLookup = new Models.vLookupList[] { new Models.vLookupList { Control = 0, Name = "None", Description = "No Procedures Available" } };
                if (res?.Count() > 0)
                {
                    if (res.Count() > 1 || res[0].ProcedureControl != 0) //if an empty object was returned we want to ignore it and return our above default vlookup object
                    {
                        newLookup = (from t in res
                                     orderby t.ProcedureName
                                     select new Models.vLookupList { Control = t.ProcedureControl, Name = t.ProcedureName, Description = t.ProcedureDescription }
                                     ).ToArray();
                    }
                }
        ...
        /// <summary>
        /// GenericResult.Control = ProcedureControl
        /// GenericResult.intField1 = UserGroupsControl
        /// </summary>
        /// <param name="data"></param>
        /// <returns></returns>
        [HttpPost, ActionName("RemoveProcedureRestrictionFromGroup")]
        public Models.Response RemoveProcedureRestrictionFromGroup([System.Web.Http.FromBody]Models.GenericResult data)
        {
            var response = new Models.Response(); //new HttpResponseMessage();
            if (!authenticateController(ref response)) { return response; }
            try
            {
                int procedureControl = data.Control;
                int groupControl = data.intField1;
                NGLSecurityData.RemoveRestrictedProcedureByGroup(procedureControl, groupControl);
