...
{
    public class TariffRateClassController : NGLControllerBase
    {
        ...
        /// </summary>
        public TariffRateClassController()
                : base(Utilities.PageEnum.TariffRatesClass)
	     {
        }
        ...
        /// </summary>
        private string _SourceClass = "DynamicsTMS365.Controllers.TariffRateClassController";
        ...
        {
            if (string.IsNullOrWhiteSpace(laneNumber) || laneNumber.ToLowerInvariant() == "all")
                return 0;
        ...
        public Models.Response Get(int id)
        {
            ...
            try
            {
                ...
                LTS.vCarrTarEquipMatPivotDetail[] oData = new LTS.vCarrTarEquipMatPivotDetail[] { };
                oData = oDAL.GetCarrTarEquipMatClassRates(f, ref RecordCount);
                if (oData == null)
                {
                    oData = new LTS.vCarrTarEquipMatPivotDetail[] { };
                    count = 0;
                }
                else
                {
        ...
        [HttpGet, ActionName("GetBreakPoint")]
        public Models.Response GetBreakPoint(int id)
        {            
            // create a response message to send back
            var response = new Models.Response(); //new HttpResponseMessage();
            if (!authenticateController(ref response)) { return response; }
            ...
            try
            {
                ...
                LTS.vCarrTarEquipMatBreakPoint[] oRecords = new LTS.vCarrTarEquipMatBreakPoint[1];
                oData = oDAL.GetCarrTarEquipMatBreakPoint(id);
                if (oData != null)
                {
                    oRecords[0] = oData;
                    count = 1;
        ...
        public Models.Response Post([System.Web.Http.FromBody]LTS.vCarrTarEquipMatPivotDetail data)
        {
            //create a response message to send back
            var response = new Models.Response(); //new HttpResponseMessage();
            if (!authenticateController(ref response)) { return response; }
            ...
            {
                DAL.NGLCarrTarEquipMatData oDAL = new DAL.NGLCarrTarEquipMatData(Parameters);
                bool blnRet = oDAL.SaveCarrTarEquipMatClassRate(data);
        ...
        public Models.Response PostSpreadSheet(LTS.vCarrTarEquipMatPivotDetail[] data, int CarrTarEquipMatCarrTarControl, int CarrTarEquipMatCarrTarMatBPControl, string CarrTarEquipMatName, DateTime EffectiveDate, DateTime? EffectiveTo = null)
        {
            //create a response message to send back
            var response = new Models.Response(); //new HttpResponseMessage();
            if (!authenticateController(ref response)) { return response; }
            try
            {
                ...
                var importData = data.Select(x => new DTO.RateImportDetailData
                {
                    ...
                    State = x.CarrTarEquipMatState,
                    Country = x.CarrTarEquipMatCountry,
                    City = string.IsNullOrEmpty(x.CarrTarEquipMatCity) ? null : x.CarrTarEquipMatCity,
                    ...
                    TariffClass = x.CarrTarEquipMatClass,
                    Val1 = x.Val1 ?? default,
                    Val2 = x.Val2 ?? default,
                    Val3 = x.Val3 ?? default,
                    Val4 = x.Val4 ?? default,
                ...
                if (oRet.Errors?.Count > 0)
                    throw new InvalidOperationException("Failed to improt tariff rates");
        ...
        //[HttpDelete, ActionName("DeleteCarrTarEquipMatRecords")]
        //public Models.Response DeleteCarrTarEquipMatRecords(string carrTarEquipMatControl)
        //{
        //    //create a response message to send back
        //    var response = new Models.Response(); //new HttpResponseMessage();
        //    if (!authenticateController(ref response)) { return response; }
