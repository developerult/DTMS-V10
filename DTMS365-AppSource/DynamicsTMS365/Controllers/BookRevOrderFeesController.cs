...
{
    public class BookRevOrderFeesController : NGLControllerBase
    {
        ...
        /// </summary>
        public BookRevOrderFeesController()
        ...
        /// </summary>
        private string _SourceClass = "DynamicsTMS365.Controllers.BookRevOrderFeesController";
        ...
        public Models.Response GetAllRecords(string filter)
        {
            ...
            try
            {
                ...
                DAL.NGLBookFeeData oBookFeesDAL = new DAL.NGLBookFeeData(Parameters);
                int count = 0;
                fees = oBookFeesDAL.GetvBookFeesForOrder(iBookControl);
        ...
        public Models.Response Post([System.Web.Http.FromBody] Models.vBookFee data)
        {
            ...
            try
            {
                ...
                if (data.BFControl == 0)
                {
                    sPageTitle = "Create new Order Specific Fee";
                    if  (data.BFBookControl == 0)
                    {
                        //Modified by RHR for v-8.5.4.004 on 12/06/2023 new BookControl setting
                        //data.BFBookControl = readPagePrimaryKey(Parameters, Utilities.PageEnum.LoadBoard);
                        int iBookControl = 0;
                        iBookControl = readBookControlPageSetting(iBookControl);
                        if (iBookControl == 0)
                        {
                        ...
                        data.BFBookControl = iBookControl;
                    }
                    
                    data.BFAccessorialFeeTypeControl = 3;
                    DTO.BookFee[] oNewFees = new DTO.BookFee[] { dtoData };
                    //oNewFees[0] = Models.vBookFee.selectDTOData(data);
                    oData = oBLL.CreateBookFeesD365(oNewFees);
                } else
                {
                    sPageTitle = "Update Order Specific Fee";
