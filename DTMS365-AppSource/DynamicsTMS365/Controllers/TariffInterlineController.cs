...
{
    public class TariffInterlineController : NGLControllerBase
    {
        ...
        /// </summary>
        public TariffInterlineController()
        ...
        /// </summary>
        private string _SourceClass = "DynamicsTMS365.Controllers.TariffInterlineController";
        ...
        public Models.Response Get(int id)
        {
            ...
            try
            {
                ...
                if (id != 0)
                {
                    f.filterName = "CarrTarInterlineControl";
                    f.filterValue = id.ToString();
                }
                //get the parent control
                f.ParentControl = readPagePrimaryKey(Parameters, Utilities.PageEnum.Tariff);
                DAL.NGLCarrTarInterlineData oDAL = new DAL.NGLCarrTarInterlineData(Parameters);
                DAL.Models.mCarrierTariffInterline[] oData = new DAL.Models.mCarrierTariffInterline[] { };
                oData = oDAL.GetCarrierTariffInterlines(f, ref RecordCount);
                count = oData.Count();
        ...
        public Models.Response GetAllRecords(string filter)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(filter)) { savePageFilters(filter, "TariffInterlineFilte"); }
                f.ParentControl = readPagePrimaryKey(Parameters, Utilities.PageEnum.Tariff);
        ...
        public Models.Response Post([System.Web.Http.FromBody]DAL.Models.mCarrierTariffInterline data)
        {
            //create a response message to send back
            var response = new Models.Response(); //new HttpResponseMessage();
            if (!authenticateController(ref response)) { return response; }
            ...
            {
                DAL.NGLCarrTarInterlineData oDAL = new DAL.NGLCarrTarInterlineData(Parameters);
                bool blnRet = oDAL.SaveCarrierTariffInterline(data);
        ...
        public Models.Response DELETE(int id)
        {
            ...
            {
                DAL.NGLCarrTarInterlineData oDAL = new DAL.NGLCarrTarInterlineData(Parameters);
                bool blnRet = oDAL.DeleteCarrierTariffInterline(id);
                bool[] oRecords = new bool[1];
                oRecords[0] = blnRet;
                response = new Models.Response(oRecords, 1);
            }
