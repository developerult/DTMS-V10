...
{
    public class MailLogController : NGLControllerBase
    {

        #region " Constructors "

        public MailLogController()
            : base(Utilities.PageEnum.MailLog)
        {
        }
        ...
        /// </summary>
        private string _SourceClass = "DynamicsTMS365.Controllers.MailLogController";
        ...
        public Models.Response Get(int id)
        {
            ...
            try
            {
                ...
                if (id != 0)
                {
                    f.filterName = "MailControl";
                    f.filterValue = id.ToString();
                }

                DAL.NGLEmailData oDAL = new DAL.NGLEmailData(Parameters);
                LTS.Mail[] records = new LTS.Mail[] { };
                records = oDAL.getMailLog(f, ref RecordCount);
        ...
        public Models.Response Post([System.Web.Http.FromBody] LTS.Mail data)
        {
            //create a response message to send back
            var response = new Models.Response(); //new HttpResponseMessage();
            if (!authenticateController(ref response)) { return response; }
            ...
            {
                DAL.NGLEmailData oDAL = new DAL.NGLEmailData(Parameters);
                bool blnRet = oDAL.UpdateMail(data);
        ...
        public Models.Response DELETE(int id)
        {
            ...
            {
                DAL.NGLEmailData oDAL = new DAL.NGLEmailData(Parameters);
                bool blnRet = oDAL.DeleteMail(id);
                bool[] oRecords = new bool[1];
                oRecords[0] = blnRet;
                response = new Models.Response(oRecords, 1);
            }
