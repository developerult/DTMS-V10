...
{
    public class LEUserFormController : NGLControllerBase
    {
        #region " Constructors "
        ...
        /// </summary>
        private string _SourceClass = "DynamicsTMS365.Controllers.LEUserFormController";
        ...
        private Models.Form selectModelData(LTS.vRestrictedFormsByGroup d)
        {
        ...
        public Models.Response GetByParent(int id)
        {
            ...
            try
            {
                //save the GroupControl so it can be used by the method RemoveFormRestrictionFromGroup()
                string msg = "";
                savePageSetting(id.ToString(), ref msg, "FormParentRoleControl");

                Models.Form[] records = new Models.Form[] { };
                int RecordCount = 0;
                int count = 0;
                DAL.Models.AllFilters f = new DAL.Models.AllFilters { ParentControl = id };
                LTS.vRestrictedFormsByGroup[] oData = NGLUserGroupData.GetRestrictedFormsByGroup(ref RecordCount, f);
                if (oData?.Count() > 0)
                {
                    count = oData.Count();
        ...
        [HttpGet, ActionName("GetUnRestrictedFormsByGroup")]
        public Models.Response GetUnRestrictedFormsByGroup()
        {
            var response = new Models.Response(); //new HttpResponseMessage();
            if (!authenticateController(ref response)) { return response; }
            try
            {
                var sSetting = readPageSettings("FormParentRoleControl", Parameters, Utilities.PageEnum.LEUsers);
                int groupControl = 0;
                int.TryParse(sSetting[0].UserPSMetaData, out groupControl);

                DTO.tblFormList[] res = NGLSecurityData.GetUnRestrictedFormsByGroup(groupControl);
                Models.vLookupList[] newLookup = new Models.vLookupList[] { new Models.vLookupList { Control = 0, Name = "None", Description = "No Screens Available" } };
                if (res != null && res.Count() > 0)
                {
                    if (res.Count() > 1 || res[0].FormControl != 0) //if an empty object was returned we want to ignore it and return our above default vlookup object
                    {
                        newLookup = (from t in res
                                     orderby t.FormName
                                     select new Models.vLookupList { Control = t.FormControl, Name = t.FormName, Description = t.FormDescription }
                                 ).ToArray();
                    }
                }
        ...
        public Models.Response Post([System.Web.Http.FromBody]Models.Form data)
        {
            var response = new Models.Response(); //new HttpResponseMessage();
            if (!authenticateController(ref response)) { return response; }
            try
            {
                if (data.FormControl > 0)
                {
                    int[] formControls = new int[1] { data.FormControl };
                    NGLUserGroupData.CreateRestrictedFormByGroup(formControls, data.UserGroupsControl);
                }
        ...
        /// <summary>
        /// GenericResult.Control = FormControl
        /// GenericResult.intField1 = UserGroupsControl
        /// </summary>
        /// <param name="data"></param>
        /// <returns></returns>
        [HttpPost, ActionName("RemoveFormRestrictionFromGroup")]
        public Models.Response RemoveFormRestrictionFromGroup([System.Web.Http.FromBody]Models.GenericResult data)
        {
            var response = new Models.Response(); //new HttpResponseMessage();
            if (!authenticateController(ref response)) { return response; }
            try
            {
                int formControl = data.Control;
                int groupControl = data.intField1;
                NGLSecurityData.RemoveRestrictedFormByGroup(formControl, groupControl);
