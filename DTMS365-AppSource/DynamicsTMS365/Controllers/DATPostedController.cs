...
{
    public class DATPostedController : NGLControllerBase
    {

        #region " Constructors "

        /// <summary>Initializes the Page property by calling the base class constructor</summary>
        public DATPostedController() : base(Utilities.PageEnum.DAT) { }
        ...
        private string _SourceClass = "DynamicsTMS365.Controllers.DATPostedController";
        ...
        public Models.Response GetAllRecords(string filter)
        {
            ...
            try
            {
                if (!string.IsNullOrWhiteSpace(filter)) { savePageFilters(filter, "DATPostFltr"); } //save the page filter for the next time the page loads            
                DAL.Models.AllFilters f = new System.Web.Script.Serialization.JavaScriptSerializer().Deserialize<DAL.Models.AllFilters>(filter);
                LTS.vDATPosted[] oData = new LTS.vDATPosted[] { };
                int RecordCount = 0;
                int count = 0;
                oData = NGLLoadTenderData.GetDATPosted(ref RecordCount, f);
                if (oData?.Count() > 0)
                {
                    count = oData.Count();
