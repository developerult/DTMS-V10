...
{
    public class MultiPickConsController : NGLControllerBase
    {

        #region " Constructors "

        public MultiPickConsController()
            : base(Utilities.PageEnum.MultiPickCons)
        {
        }
        ...
        /// </summary>
        private string _SourceClass = "DynamicsTMS365.Controllers.MultiPickConsController";
        ...
        public Models.Response Get(int id)
        {
            ...
            try
            {
                ...
                records = oDAL.GetCarrierInsExpAllShipAlerts365(f, ref RecordCount);
        ...
        public Models.Response GetAllRecords(string filter)
        {
            ...
            try
            {
                ...
                DAL.NGLvBookMultiPickData oDAL = new DAL.NGLvBookMultiPickData(Parameters);
                Models.vBookMultiPick[] records = new Models.vBookMultiPick[] { };
                string sMultiPickConsPrefix = "";
                DAL.Models.FilterDetails oDetails = f.FilterValues.Where(x => x.filterName == "MultiPickConsPrefix").FirstOrDefault();
                if (oDetails != null)
                {

                    sMultiPickConsPrefix = oDetails.filterValueFrom;
                    DTO.vBookMultiPick[] oData = new DTO.vBookMultiPick[] { };

                    oData = oDAL.GetvBookMultiPickRecords(sMultiPickConsPrefix);
                    if (oData != null && oData.Count() > 0)
                    {
                        count = oData.Count();
                        if (RecordCount > count) { count = RecordCount; }
                        records = (from e in oData select Models.vBookMultiPick.selectModelData(e)).ToArray();
                    }
                }
