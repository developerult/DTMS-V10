...
{
    public class UserLaneController : NGLControllerBase
    {
        ...
        /// </summary>
        private string _SourceClass = "DynamicsTMS365.Controllers.UserLaneController";
        ...
        private Models.vUserLane selectModelData(LTS.vUserLane365 d)
        {
            Models.vUserLane modelRecord = new Models.vUserLane();
            if (d != null)
            {
                List<string> skipObjs = new List<string> { "USLUpdated","USLModDate" };
                string sMsg = "";
                modelRecord = (Models.vUserLane)DTran.CopyMatchingFields(modelRecord, d, skipObjs, ref sMsg);
                if (modelRecord != null) {
                    modelRecord.setUpdated(d.USLUpdated.ToArray());
                    if (d.USLModDate.HasValue)
                    {
                        modelRecord.USLModDate = d.USLModDate.Value;
                    } else
                    {
                        modelRecord.USLModDate = DateTime.Now;
                    }
        ...
        public Models.Response GetByParent(int id)
        {
            ...
            try
            {
                Models.vUserLane[] records = new Models.vUserLane[] { };
                int RecordCount = 0;
                int count = 0;
                DAL.Models.AllFilters f = new DAL.Models.AllFilters { ParentControl = id };
                LTS.vUserLane365[] oData = NGLUserLaneData.GetvUserLanesByUser(ref RecordCount, f);
                if (oData?.Count() > 0)
                {
                    count = oData.Count();
        ...
        public Models.Response DELETE(int id)
        {
            ...
            try
            {
                bool blnRet = NGLUserLaneData.DeleteUserLane(id);
        ...
        [HttpGet, ActionName("GetUnrestrictedLanesByUser")]
        public Models.Response GetUnrestrictedLanesByUser(int id)
        {
            var response = new Models.Response(); //new HttpResponseMessage();
            if (!authenticateController(ref response)) { return response; }
            try
            {
                int count = 0;
                LTS.spGetUnrestrictedLanesByUserResult[] records = NGLUserLaneData.GetUnrestrictedLanesByUser(id);
                if (records?.Count() > 0) { count = records.Count(); } else { records = new LTS.spGetUnrestrictedLanesByUserResult[] { }; }
                response = new Models.Response(records, count);
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// GenericResult.intArray = An array of LaneNumbers
        /// GenericResult.Control = USC
        /// </summary>
        /// <param name="data"></param>
        /// <returns></returns>
        [HttpPost, ActionName("AddUserLanes")]
        public Models.Response AddUserLanes([System.Web.Http.FromBody]Models.GenericResult data)
        {
            var response = new Models.Response(); //new HttpResponseMessage();
            if (!authenticateController(ref response)) { return response; }
            try
            {
                bool blnRet = NGLUserLaneData.InsertUserLane(data.intArray, data.Control);
                bool[] oRecords = new bool[1] { blnRet };
                response = new Models.Response(oRecords, 1);
            }
            catch (Exception ex)
