...
namespace DynamicsTMS365.Controllers
{
    public class LoadTypeController : NGLControllerBase
    {
        ...
        /// </summary>
        private string _SourceClass = "DynamicsTMS365.Controllers.LoadTypeController";
        ...
        private Models.vLoadType selectModelData(LTS.LoadType d)
        {
            Models.vLoadType modelRecord = new Models.vLoadType();
            if (d != null)
            {
                List<string> skipObjs = new List<string> { "LoadTypeUpdated", "rowguid" };
                string sMsg = "";
                modelRecord = (Models.vLoadType)Ngl.Core.Utility.DataTransformation.CopyMatchingFields(modelRecord, d, skipObjs, ref sMsg);
                ...
                if (modelRecord != null) { modelRecord.setUpdated(d.LoadTypeUpdated.ToArray()); }
            }
        ...
        public Models.Response Get(int id)
        {
            ...
            try
            {
                ...
                DAL.NGLLoadTypeData oDAL = new DAL.NGLLoadTypeData(Parameters);
                LTS.LoadType oData = new LTS.LoadType { };
                oData = oDAL.GetLoadTypebyFiltered(id);
                Models.vLoadType[] records = new Models.vLoadType[1];
                if (oData != null)
                {
                    count = 1;
        ...
        public Models.Response GetAllRecords(string filter)
        {
            ...
            try
            {
                ...
                Models.vLoadType[] records = new Models.vLoadType[] { };
                LTS.LoadType[] oData = new LTS.LoadType[] { };
                oData = oDAL.GetLoadTypeFiltered(f, ref RecordCount);
        ...
        public Models.Response Post([System.Web.Http.FromBody]LTS.LoadType data)
        {
            var response = new Models.Response(); //new HttpResponseMessage();
            if (!authenticateController(ref response)) { return response; }
            try
            {
                DAL.NGLLoadTypeData oDAL = new DAL.NGLLoadTypeData(Parameters);
                bool blnRet = oDAL.SaveOrCreateLoadType(data);
        ...
        public Models.Response DELETE(int id)
        {
            ...
            {
                DAL.NGLLoadTypeData oDAL = new DAL.NGLLoadTypeData(Parameters);
                bool blnRet = oDAL.DeleteLoadType(id);
                bool[] oRecords = new bool[1];
                oRecords[0] = blnRet;
                response = new Models.Response(oRecords, 1);
            }
