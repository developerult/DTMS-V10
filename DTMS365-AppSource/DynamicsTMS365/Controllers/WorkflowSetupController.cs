...
{
    /// <summary>
    /// Main Display for Workflow Setup page links to tblParCategory
    /// </summary>
    /// <remarks>
    /// Created by RHR for v-8.5.3.006 on 10/20/2022
    /// </remarks>
    public class WorkflowSetupController : NGLControllerBase
    {
        #region " Constructors "
        ...
        /// </summary>
        private string _SourceClass = "DynamicsTMS365.Controllers.WorkflowSetupController";
        ...
        public Models.Response Get(int id)
        {
            ...
            try
            {
                ...
                if (id != 0)
                {
                    f.filterName = "ParCatControl";
                    f.filterValue = id.ToString();
                }

                DAL.NGLParCategoryData oDAL = new DAL.NGLParCategoryData(Parameters);
                Models.tblParCategory[] records = new Models.tblParCategory[] { };
                LTS.tblParCategory[] oData = new LTS.tblParCategory[] { };
                oData = oDAL.GettblParCategories(f, ref RecordCount);
                //if (oData != null && oData.Count() > 0)
                //{
                count = oData.Count();
                records = (from e in oData select Models.tblParCategory.selectModelData(e)).ToArray();
        ...
        public Models.Response Post([System.Web.Http.FromBody] Models.tblParCategory data)
        {
            //create a response message to send back
            var response = new Models.Response(); //new HttpResponseMessage();
            if (!authenticateController(ref response)) { return response; }
            ...
            {
                DAL.NGLParCategoryData oDAL = new DAL.NGLParCategoryData(Parameters);
                LTS.tblParCategory oChanges = Models.tblParCategory.selectLTSData(data);
                bool blnRet = oDAL.SavetblParCategory(oChanges);
        ...
        public Models.Response DELETE(int id)
        {
            ...
            {
                DAL.NGLParCategoryData oDAL = new DAL.NGLParCategoryData(Parameters);
                bool blnRet = oDAL.DeletetblParCategory(id);
