...
{
    public class TariffDiscountController : NGLControllerBase
    {
        ...
        /// </summary>
        public TariffDiscountController()
        ...
        /// </summary>
        private string _SourceClass = "DynamicsTMS365.Controllers.TariffDiscountController";
        ...
        public Models.Response Get(int id)
        {
            ...
            try
            {
                ...
                if (id != 0)
                {
                    f.filterName = "CarrTarDiscountControl";
                    f.filterValue = id.ToString();
                }
                //get the parent control
                f.ParentControl = readPagePrimaryKey(Parameters, Utilities.PageEnum.Tariff);
                DAL.NGLCarrTarDiscountData oDAL = new DAL.NGLCarrTarDiscountData(Parameters);                
                LTS.vCarrierTariffDiscount[] oData = new LTS.vCarrierTariffDiscount[] { };
                oData = oDAL.GetCarrierTariffDiscounts(f, ref RecordCount);
                count = oData.Count();
        ...
        public Models.Response GetAllRecords(string filter)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(filter)) { savePageFilters(filter, "TariffDiscountFilter"); }
                f.ParentControl = readPagePrimaryKey(Parameters, Utilities.PageEnum.Tariff);
        ...
        public Models.Response Post([System.Web.Http.FromBody]LTS.vCarrierTariffDiscount data)
        {
            var response = new Models.Response(); //new HttpResponseMessage();
            if (!authenticateController(ref response)) { return response; }
            try
            {
                DAL.NGLCarrTarDiscountData oDAL = new DAL.NGLCarrTarDiscountData(Parameters);
                bool blnRet = oDAL.SaveCarrierTariffDiscount(data);
        ...
        public Models.Response DELETE(int id)
        {
            ...
            {
                DAL.NGLCarrTarDiscountData oDAL = new DAL.NGLCarrTarDiscountData(Parameters);
                bool blnRet = oDAL.DeleteCarrierTariffDiscount(id);
                bool[] oRecords = new bool[1];
                oRecords[0] = blnRet;
                response = new Models.Response(oRecords, 1);
            }
