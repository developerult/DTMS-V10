...
{
    public class TariffClassXrefController : NGLControllerBase
    {
        #region " Constructors "
        /// <summary>
        /// Created by RHR for v-8.2 on 08/29/2018 initializes the Page property by calling the base class constructor
        /// </summary>
        public TariffClassXrefController()
                : base(Utilities.PageEnum.TariffExceptions)
	     {
        }
        ...
        /// </summary>
        private string _SourceClass = "DynamicsTMS365.Controllers.TariffClassXrefController";
        ...
        public Models.Response Get(int id)
        {
            ...
            try
            {
                ...
                if (id != 0)
                {
                    f.filterName = "CarrTarClassXrefControl";
                    f.filterValue = id.ToString();
                }
                //get the parent control
                f.ParentControl = readPagePrimaryKey(Parameters, Utilities.PageEnum.Tariff);
                DAL.NGLCarrTarClassXrefData oDAL = new DAL.NGLCarrTarClassXrefData(Parameters);
                LTS.vCarrierTariffClassXref[] oData = new LTS.vCarrierTariffClassXref[] { };
                oData = oDAL.GetCarrierTariffClassXrefs(f, ref RecordCount);
                if (oData == null)
                {
                    oData = new LTS.vCarrierTariffClassXref[] { };
                    count = 0;
                }
                else
                {
        ...
        public Models.Response GetAllRecords(string filter)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(filter)) { savePageFilters(filter, "TariffClassXrefFilte"); }
                f.ParentControl = readPagePrimaryKey(Parameters, Utilities.PageEnum.Tariff);
        ...
        public Models.Response Post([System.Web.Http.FromBody]LTS.vCarrierTariffClassXref data)
        {
            var response = new Models.Response(); //new HttpResponseMessage();
            if (!authenticateController(ref response)) { return response; }
            try
            {
                DAL.NGLCarrTarClassXrefData oDAL = new DAL.NGLCarrTarClassXrefData(Parameters);
                bool blnRet = oDAL.SaveCarrierTariffClassXref(data);
        ...
        public Models.Response DELETE(int id)
        {
            ...
            {
                DAL.NGLCarrTarClassXrefData oDAL = new DAL.NGLCarrTarClassXrefData(Parameters);
                bool blnRet = oDAL.DeleteCarrierTariffClassXref(id);
                bool[] oRecords = new bool[1];
                oRecords[0] = blnRet;
                response = new Models.Response(oRecords, 1);
            }
