...
{
    public class DATErrorController : NGLControllerBase
    {

        #region " Constructors "

        /// <summary>Initializes the Page property by calling the base class constructor</summary>
        public DATErrorController() : base(Utilities.PageEnum.DAT) { }
        ...
        private string _SourceClass = "DynamicsTMS365.Controllers.DATErrorController";
        ...
        public Models.Response GetAllRecords(string filter)
        {
            ...
            try
            {
                if (!string.IsNullOrWhiteSpace(filter)) { savePageFilters(filter, "DATErrFltr"); } //save the page filter for the next time the page loads            
                DAL.Models.AllFilters f = new System.Web.Script.Serialization.JavaScriptSerializer().Deserialize<DAL.Models.AllFilters>(filter);
                LTS.vDATError[] oData = new LTS.vDATError[] { };
                int RecordCount = 0;
                int count = 0;
                oData = NGLLoadTenderData.GetDATErrors(ref RecordCount, f);
                if (oData?.Count() > 0)
                {
                    count = oData.Count();
