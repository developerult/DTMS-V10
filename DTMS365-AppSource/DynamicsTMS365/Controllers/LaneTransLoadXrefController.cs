...
namespace DynamicsTMS365.Controllers
{
    public class LaneTransLoadXrefController : NGLControllerBase
    {
        ...
        /// </summary>
        private string _SourceClass = "DynamicsTMS365.Controllers.LaneTransLoadXrefController";
        ...
        public static LTS.LaneTransLoadXref selectLTSData(Models.LaneTransLoadXref d)
        {
            LTS.LaneTransLoadXref ltsRecord = new LTS.LaneTransLoadXref();
            if (d != null)
            {
                List<string> skipObjs = new List<string> { "LaneTranXUpdated",  "Lane","LaneTransLoadXrefDets", "tblModeTypeRefLane","LaneTranXLaneNumber", "LaneTranXLaneName", "LaneTranXModeTypeName", "LaneTranXTempTypeName", "TempType" };
                string sMsg = "";
                ltsRecord = (LTS.LaneTransLoadXref)Ngl.Core.Utility.DataTransformation.CopyMatchingFields(ltsRecord, d, skipObjs, ref sMsg);
                if (ltsRecord != null)
                {
                    byte[] bupdated = d.getUpdated();
                    ltsRecord.LaneTranXUpdated = bupdated == null ? new byte[0] : bupdated;
        ...
        public static LTS.vLaneTransLoadXref selectDTOData(Models.LaneTransLoadXref d)
        {
            LTS.vLaneTransLoadXref ltsRecord = new LTS.vLaneTransLoadXref();
            if (d != null)
            {
                List<string> skipObjs = new List<string> { "LaneTranXUpdated", "rowguid" };
                string sMsg = "";
                ltsRecord = (LTS.vLaneTransLoadXref)Ngl.Core.Utility.DataTransformation.CopyMatchingFields(ltsRecord, d, skipObjs, ref sMsg);
                if (ltsRecord != null)
                {
                    byte[] bupdated = d.getUpdated();
                    ltsRecord.LaneTranXUpdated = bupdated == null ? new byte[0] : bupdated;
        ...
        public Models.Response Get(int id)
        {
            ...
            try
            {
                ...
                LTS.vLaneTransLoadXref oData = oDAL.GetLaneTransLoadXref365(id);
                Models.LaneTransLoadXref[] records = new Models.LaneTransLoadXref[1];
                if (oData != null)
                {
                    count = 1;
        ...
        public Models.Response Post([System.Web.Http.FromBody]Models.LaneTransLoadXref data)
        {
            var response = new Models.Response(); //new HttpResponseMessage();
            if (!authenticateController(ref response)) { return response; }
            try
            {
                ...
                int iLaneTranXLaneControl = readPagePrimaryKey(Parameters, Utilities.PageEnum.LaneMaint);

                DAL.NGLLaneTransLoadXrefData oDAL = new DAL.NGLLaneTransLoadXrefData(Parameters);
                LTS.LaneTransLoadXref record = selectLTSData(data);
                record.LaneTranXLaneControl = iLaneTranXLaneControl;
                bool blnRet = oDAL.SaveOrCreateLaneTransLoadXrefItem(record, iLaneTranXLaneControl);
        ...
        public Models.Response DELETE(int id)
        {
            ...
            {
                DAL.NGLLaneTransLoadXrefData oDAL = new DAL.NGLLaneTransLoadXrefData(Parameters);
                bool blnRet = oDAL.DeleteLaneTransLoadXrefItem(id);
                bool[] oRecords = new bool[1];
                oRecords[0] = blnRet;
                response = new Models.Response(oRecords, 1);
            }
