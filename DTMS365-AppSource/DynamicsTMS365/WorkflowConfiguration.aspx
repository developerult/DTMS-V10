<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="WorkflowConfiguration.aspx.cs" Inherits="DynamicsTMS365.WorkflowConfiguration" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head >
    <title>DTMS Task Manager</title>        
         <%=cssReference%>             
        <style>
            html,
    ...
    <body>     
        <%=jssplitter2Scripts%> 
        <%=sWaitMessage%>         
    <div id="root" style="height: 100%; width: 100%;  margin-top: 2px;">
    ...
    <% Response.WriteFile("~/Views/HelpWindow.html"); %>   
     ...
     <script>
        ...
        var PageControl = '<%=PageControl%>'; 
         var oTaskManagerGrid = null;
         var wnd = kendo.ui.Window; 
         var tObj = this;
         var tPage = this;
         var iTaskManagerPK = 0;
        ...
        <% Response.Write(NGLOAuth2); %>
         ...
         function savePostPageSettingSuccessCallback(results){
             wdgtTaskManagerummarySummary.read(iTaskManagerPK);
         }
                             ...
                             if (data.Data.length > 0 && typeof (data.Data[0]) !== 'undefined' && ngl.isObject(data.Data[0])) {
                                 ...
                                 if(typeof (userPageSettings) !== 'undefined' && userPageSettings != null && userPageSettings.value != undefined){
                                     if (userPageSettings.name === "pk"){ 
                                         if(typeof (iTaskManagerPK) === 'undefined' || iTaskManagerPK == null || iTaskManagerPK === 0){ iTaskManagerPK = userPageSettings.value; }                                 
                                     }                                                                                                                                  
                                 }                                   
                             }                              
                         }                            
         ...
         function saveTaskManagerPK() {
             try {
                 var row = oTaskManagerGrid.select();
                 ...
                 var dataItem = oTaskManagerGrid.dataItem(row);
                 ...
                 if ("RunTaskControl" in dataItem){
                     ...
                     iTaskManagerPK = dataItem.RunTaskControl;
                     var setting = {name:'pk', value: iTaskManagerPK.toString()};
                     var oCRUDCtrl = new nglRESTCRUDCtrl();
                     var blnRet = oCRUDCtrl.update("TaskManager/PostPageSetting", setting, tPage, "savePostPageSettingSuccessCallback", "savePostPageSettingSuccessCallbackAjaxErrorCallback",tPage);
                     return true;
                 } else {
                     ngl.showValidationMsg("TMS Task Required", "Please select a TMS Task Record to continue", tPage);
                     return false;
         ...
         function isTaskManagerelected() {
             if (typeof (iTaskManagerPK) === 'undefined' || iTaskManagerPK === null || iTaskManagerPK === 0) {
                 return saveTaskManagerPK();
             }
             return true;
         }
         function execActionClick(btn, proc){            
             if(btn.id == "btnRefresh" ){ 
                 oTaskManagerGrid.dataSource.read();
             } else if(btn.id == "btnResetCurrentUserConfig"){ resetCurrentUserConfig(PageControl); }
         }
         var blnTaskManagerGridChangeBound = false;
         //*************  Call Back Functions ****************
         function TaskManagerGridDataBoundCallBack(e,tGrid){           
             oTaskManagerGrid = tGrid;
             if (blnTaskManagerGridChangeBound == false){
                 oTaskManagerGrid.bind("change", saveTaskManagerPK);
                 blnTaskManagerGridChangeBound = true;
             }
             //if iTaskManagerPK is not 0 select that row in the grid
             if (typeof (iTaskManagerPK) !== 'undefined' && iTaskManagerPK !== null && iTaskManagerPK !== 0) {
                 var rows = oTaskManagerGrid.items();
                 $(rows).each(function(e) {
                     var row = this;
                     var dataItem = oTaskManagerGrid.dataItem(row);
                     if (dataItem.RunTaskControl == iTaskManagerPK) { 
                         oTaskManagerGrid.select(row); 
                         return false;
                     }
                 });
             }
             ...
             //lblBalanceID =  wdgtTaskManagerGridEdit.GetFieldID("lblBalanceTotal");
             //wdgtTaskManagerGridEdit.SetFieldDefault("lblBalanceTotal",'$100.00');
         ...
         $(document).ready(function () {
            ...
            if (control != 0){
                ...
                getPageSettings(tPage, "TaskManager", "pk", false);
            }
