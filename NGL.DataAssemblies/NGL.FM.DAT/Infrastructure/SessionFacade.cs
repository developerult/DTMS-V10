...
namespace NGL.FM.DAT.Infrastructure
{
	...
	public class SessionFacade
	{
            ...
            TfmiFreightMatchingPortTypeClient client)
        {
            ...
            //_sessionHeader = BuildSessionHeader(loginSuccessData);
            //LVV CHANGE
            _sessionHeader = BuildSessionHeaderFromToken(token);
            _client = client;
        }
        ...
        /// <summary>
        /// 	Calls <see cref="TfmiFreightMatchingPortTypeClient.DeleteAsset" /> method and writes result to console.
        /// </summary>
        /// <param name="strIDs"></param>
        /// <param name="lt"></param>
        /// <param name="oWCF"></param>
        public DTO.DATResults DeleteAssetByAssetIDs(string[] strIDs, DTO.tblLoadTender lt, DAL.WCFParameters oWCF)
        {
            var datReturn = new DTO.DATResults();

            var deleteAssetOperation = new DeleteAssetOperation { Item = new DeleteAssetsByAssetIds{assetIds = strIDs} };
            var deleteAssetRequest = new DeleteAssetRequest { deleteAssetOperation = deleteAssetOperation };
            ...
            Console.WriteLine("[deleting assets by Asset IDs] " + strIDs.ToString());

            Console.WriteLine("===============Delete Results===============");

            if (deleteAssetResponse != null)
            {
                DeleteAssetResult deleteAssetResult = deleteAssetResponse.deleteAssetResult;
                
                    var deleteAssetSuccessData = deleteAssetResult.Item as DeleteAssetSuccessData;
                    if (deleteAssetSuccessData == null)
                    {
                        var serviceError = deleteAssetResult.Item as ServiceError;
                        //serviceError.Display(); // Remember to remove this line for production
                        string strServiceError = serviceError.GetString();
                        // add it to the log
                        var oSysData = new DAL.NGLSystemLogData(oWCF);
                        string source = "NGL.FM.DATIntegration.SessionFacade.DeleteAssetByAssetIDs";
                        oSysData.AddApplicaitonLog(strServiceError, source);

                        var p = new string[] { strServiceError };

                        var datRet = new DTO.DATResults {
                            Success = false,
                            LTStatusCode = DTO.tblLoadTender.LoadTenderStatusCodeEnum.DATError,
                            LTMessage = strServiceError,
                            LTControl = lt.LoadTenderControl,
                            UserName = lt.UserName,
                            LTBookControl = lt.LTBookControl,
                            LTDATRefID = strIDs[0],
                            LTCarrierName = lt.LTCarrierName,
                            LTCarrierNumber = lt.LTCarrierNumber,
                            LTCarrierControl = lt.LTCarrierControl,
                            LTBookCustCompControl = lt.LTBookCustCompControl,
                            LTCompName = lt.LTCompName,
                            LTBookSHID = lt.LTBookSHID,
                            LTTypeControl = lt.LTLoadTenderTypeControl};

                        datRet.AddMessage(DTO.DATResults.MessageType.Warnings, DTO.DATResults.MessageEnum.E_DATDeletePostServiceError, p);
                        datReturn = datRet;
                    }
                    else
                    {
                        string strWarnings = "";
                        if (deleteAssetSuccessData.warnings != null) {
                            for (int i = 0; i < deleteAssetSuccessData.warnings.Length; i++)
                            {
                                strWarnings += deleteAssetSuccessData.warnings[i].code.ToString() + ": " + deleteAssetSuccessData.warnings[i].message;
                            }
                        }
                        ...
                        {
                            Success = true,
                            LTMessage = strWarnings,
                            LTStatusCode = DTO.tblLoadTender.LoadTenderStatusCodeEnum.DATDeleted,
                            BookTrackComment = "Post with Reference ID " + strIDs[0] + " Deleted from DAT.",
                            BookTrackStatus = 501,
                            LTControl = lt.LoadTenderControl,
                            UserName = lt.UserName,
                            LTBookControl = lt.LTBookControl,
                        ...
                        if (!string.IsNullOrWhiteSpace(strWarnings))
                        {
                            var p = new string[] { strWarnings };
                            datRet.AddMessage(DTO.DATResults.MessageType.Warnings, DTO.DATResults.MessageEnum.E_DATGeneralRetMsg, p);                       
                        }                  
                        datReturn = datRet;
                    }             
            }
        ...
        /// <param name="lt"></param>
        /// <param name="oWCF"></param>
        public DTO.DATResults Post(PostAssetRequest postAssetRequest, DTO.tblLoadTender lt, DAL.WCFParameters oWCF)
		{
            var datReturn = new DTO.DATResults();
            int LoadTenderControl = lt.LoadTenderControl;
            int BookControl = lt.LTBookControl;
			/* pass a local variable as a "ref" parameter, rather than passing the field itself, so 
			 * the service can't modify what the field refers to */
			CorrelationHeader correlationHeader = _correlationHeader;
			SessionHeader sessionHeader = _sessionHeader;
			...
			if (postAssetResponse != null)
			{
				foreach (PostAssetResult postAssetResult in postAssetResponse.postAssetResults)
				{
					...
					if (postAssetSuccessData == null)
					{
                        ...
                        // add it to the log
                        var oSysData = new DAL.NGLSystemLogData(oWCF);
                        string source = "NGL.FM.DATIntegration.SessionFacade.Post";
                        oSysData.AddApplicaitonLog(strServiceError, source);
                        ...
                        var datRet = new DTO.DATResults { 
                            ...
                            LTTypeControl = lt.LTLoadTenderTypeControl};
                        var p = new string[] { strServiceError };
                        datRet.AddMessage(DTO.DATResults.MessageType.Warnings, DTO.DATResults.MessageEnum.E_DATPostFailedServiceError, p);
					...
					else
					{
                        var datRet = new DTO.DATResults {
                            Success = true,
                            LTStatusCode = DTO.tblLoadTender.LoadTenderStatusCodeEnum.DATPosted,
                            LTDATRefID = postAssetSuccessData.assetId,
                            BookTrackComment = "Load Posted to DAT. Reference ID " + postAssetSuccessData.assetId.ToString(),
                            BookTrackStatus = 500,
                            LTControl = LoadTenderControl,
                            UserName = lt.UserName,
                            LTBookControl = BookControl,
        ...
        /// <summary>
        /// 	Calls <see cref="TfmiFreightMatchingPortTypeClient.UpdateAsset" /> method and writes result to console.
        /// </summary>
        /// <param name="updateAssetRequest"></param>
        /// <param name="lt"></param>
        /// <param name="oWCF"></param>
        public DTO.DATResults UpdatePost(UpdateAssetRequest updateAssetRequest, DTO.tblLoadTender lt, DAL.WCFParameters oWCF)
        {
            var datReturn = new DTO.DATResults();
            int LoadTenderControl = lt.LoadTenderControl;
            int BookControl = lt.LTBookControl;
            ...
            Console.WriteLine("===============Update Post Results===============");

            if (updateAssetResponse != null)
            {
                UpdateAssetResult updateAssetResult = updateAssetResponse.updateAssetResult;

                var updateAssetSuccessData = updateAssetResult.Item as UpdateAssetSuccessData;
                if (updateAssetSuccessData == null)
                {
                    var serviceError = updateAssetResult.Item as ServiceError;
                    serviceError.Display(); // Remember to remove this line for production
                    string strServiceError = serviceError.GetString();
                    // add it to the log
                    var oSysData = new DAL.NGLSystemLogData(oWCF);
                    string source = "NGL.FM.DATIntegration.SessionFacade.UpdatePost";
                    oSysData.AddApplicaitonLog(strServiceError, source);               
                    ...
                    var p = new string[] { lt.LTBookSHID, lt.LTDATRefID, strServiceError };
                    datRet.AddMessage(DTO.DATResults.MessageType.Warnings, DTO.DATResults.MessageEnum.E_DATUpdatePostFailedServiceError, p);
                    datReturn = datRet;                                 
                }
                else
                {
                    string strWarnings = "";
                    if (updateAssetSuccessData.warnings != null)
                    {
                        for (int i = 0; i < updateAssetSuccessData.warnings.Length; i++)
                        {
                            strWarnings += updateAssetSuccessData.warnings[i].code.ToString() + ": " + updateAssetSuccessData.warnings[i].message;
                        }
                    }
                    ...
                    {
                        Success = true,
                        LTStatusCode = DTO.tblLoadTender.LoadTenderStatusCodeEnum.DATUpdated,
                        LTMessage = strWarnings,
                        BookTrackComment = "Post with Reference ID " + lt.LTDATRefID + " Updated on DAT.",
                        BookTrackStatus = 501,
                        LTControl = lt.LoadTenderControl,
                        UserName = lt.UserName,
                        LTBookControl = lt.LTBookControl,
        ...
        //LVV CHANGE
        private static SessionHeader BuildSessionHeaderFromToken(SessionToken token)
        {
            return new SessionHeader
            {
                sessionToken = token
